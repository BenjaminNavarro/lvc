...
#ifndef LVC_TESTING_SUPPORT_SUPPORTHELPERS_H
#define LVC_TESTING_SUPPORT_SUPPORTHELPERS_H
...
#include "lvc/Support/raw_os_ostream.h"
...
public:
  ...
  template <class T>
  operator ::testing::Matcher<const lvc::Optional<T> &>() const {
    return ::testing::MakeMatcher(
        new Impl<T>(::testing::SafeMatcherCast<T>(ValueMatcher)));
  }

  template <class T>
  class Impl : public ::testing::MatcherInterface<const lvc::Optional<T> &> {
  public:
    explicit Impl(const ::testing::Matcher<T> &ValueMatcher)
        : ValueMatcher(ValueMatcher) {}

    bool MatchAndExplain(const lvc::Optional<T> &Input,
...
/// Matches an lvc::Optional<T> with a value that conforms to an inner matcher.
/// To match lvc::None you could use Eq(lvc::None).
...
public:
  ...
  ///               lvc::sys::fs::createUniqueDirectory.
  explicit TempDir(StringRef Name, bool Unique = false) {
    std::error_code EC;
    if (Unique) {
      EC = lvc::sys::fs::createUniqueDirectory(Name, Path);
      if (!EC) {
        // Resolve any symlinks in the new directory.
        std::string UnresolvedPath(Path.str());
        EC = lvc::sys::fs::real_path(UnresolvedPath, Path);
      }
    } else {
      Path = Name;
      EC = lvc::sys::fs::create_directory(Path);
    }
    if (EC)
      Path.clear();
  ...
  ~TempDir() {
    if (!Path.empty()) {
      EXPECT_FALSE(lvc::sys::fs::remove_directories(Path.str()));
    }
  }
  ...
  SmallString<128> path(StringRef component) const {
    ...
    lvc::sys::path::native(ComponentToAppend);
    lvc::sys::path::append(Result, Twine(ComponentToAppend));
    return Result;
  }
};
...
public:
  ...
  ~TempLink() {
    if (!Path.empty()) {
      EXPECT_FALSE(lvc::sys::fs::remove(Path.str()));
    }
  }
...
public:
  ...
  ///               lvc::sys::fs::createTemporaryFile.
  TempFile(StringRef Name, StringRef Suffix = "", StringRef Contents = "",
           bool Unique = false) {
    std::error_code EC;
    int fd;
    if (Unique) {
      EC = lvc::sys::fs::createTemporaryFile(Name, Suffix, fd, Path);
    } else {
      Path = Name;
      if (!Suffix.empty()) {
        Path.append(".");
      ...
      EC = lvc::sys::fs::openFileForWrite(Path, fd);
    }
    EXPECT_FALSE(EC);
