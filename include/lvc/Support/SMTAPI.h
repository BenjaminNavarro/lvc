...
#ifndef LVC_SUPPORT_SMTAPI_H
#define LVC_SUPPORT_SMTAPI_H
...
public:
  ...
  virtual void Profile(lvc::FoldingSetNodeID &ID) const = 0;

  bool operator<(const SMTSort &Other) const {
    lvc::FoldingSetNodeID ID1, ID2;
    Profile(ID1);
    Other.Profile(ID2);
    return ID1 < ID2;
  }
...
public:
  ...
  SMTSortRef getFloatSort(unsigned BitWidth) {
    ...
    lvc_unreachable("Unsupported floating-point bitwidth!");
  }
  ...
  virtual lvc::APSInt getBitvector(const SMTExprRef &Exp, unsigned BitWidth,
                                   bool isUnsigned) = 0;
  ...
  virtual SMTExprRef mkFloat(const lvc::APFloat Float) = 0;

  /// Constructs an SMTExprRef from an APSInt and its bit width
  virtual SMTExprRef mkBitvector(const lvc::APSInt Int, unsigned BitWidth) = 0;

  /// Given an expression, extract the value of this operand in the model.
  virtual bool getInterpretation(const SMTExprRef &Exp, lvc::APSInt &Int) = 0;
