...
#ifndef LVC_SUPPORT_ERROR_H
#define LVC_SUPPORT_ERROR_H
...
#if LVC_ENABLE_ABI_BREAKING_CHECKS
    if (LVC_UNLIKELY(!getChecked() || getPtr()))
      fatalUncheckedError();
#endif
  }
...
/// error class type.
template <class T> class LVC_NODISCARD Expected {
...
/// ErrorHandling.h.
LVC_ATTRIBUTE_NORETURN void report_fatal_error(Error Err,
...
/// errors after running the handlers, or lvc_unreachable is called).
template <typename... HandlerTs>
...
/// If E is an error, lvc_unreachable will be called.
...
///   @code{.cpp}
///   Expected<int> foo() {
///      return lvc::make_error<PDBError>(pdb_error_code::dia_failed_loading,
