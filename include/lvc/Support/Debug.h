...
//
// In particular, just wrap your code with the LVC_DEBUG() macro, and it will
// be enabled automatically if you specify '-debug' on the command-line.
// LVC_DEBUG() requires the DEBUG_TYPE macro to be defined. Set it to "foo"
...
// the LVC_DEBUG() macro. Then, on the command line, you can specify
// '-debug-only=foo' to enable JUST the debug information for the foo class.
//
// When compiling without assertions, the -debug-* options and all code in
// LVC_DEBUG() statements disappears, so it does not affect the runtime of the
// code.
//
//===----------------------------------------------------------------------===//

#ifndef LVC_SUPPORT_DEBUG_H
#define LVC_SUPPORT_DEBUG_H
...
#define DEBUG_WITH_TYPE(TYPE, X)                                               \
  do {                                                                         \
    if (::lvc::DebugFlag && ::lvc::isCurrentDebugType(TYPE)) {                 \
      X;                                                                       \
    }                                                                          \
  } while (false)
...
//
// LVC_DEBUG(dbgs() << "Bitset contains: " << Bitset << "\n");
//
#define LVC_DEBUG(X) DEBUG_WITH_TYPE(DEBUG_TYPE, X)

} // end namespace lvc

#endif // LVC_SUPPORT_DEBUG_H
