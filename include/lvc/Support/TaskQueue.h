...
#ifndef LVC_SUPPORT_TASKQUEUE_H
#define LVC_SUPPORT_TASKQUEUE_H

#include "lvc/Config/llvm-config.h"
#include "lvc/Support/ThreadPool.h"
...
public:
  ...
  std::future<std::result_of_t<Callable()>> async(Callable &&C) {
    ...
    static_assert(false,
                  "TaskQueue requires building with LVC_ENABLE_THREADS!");
#endif
