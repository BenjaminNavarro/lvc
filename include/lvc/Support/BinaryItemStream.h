...
#ifndef LVC_SUPPORT_BINARYITEMSTREAM_H
#define LVC_SUPPORT_BINARYITEMSTREAM_H
...
public:
  explicit BinaryItemStream(lvc::support::endianness Endian)
      : Endian(Endian) {}
...
private:
  ...
  Expected<uint32_t> translateOffsetIndex(uint32_t Offset) {
    ...
    if (Offset >= getLength())
      return make_error<BinaryStreamError>(stream_error_code::stream_too_short);
    ++Offset;
    auto Iter = lvc::lower_bound(ItemEndOffsets, Offset);
...
#endif // LVC_SUPPORT_BINARYITEMSTREAM_H
