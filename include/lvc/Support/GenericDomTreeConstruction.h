...
#ifndef LVC_SUPPORT_GENERICDOMTREECONSTRUCTION_H
#define LVC_SUPPORT_GENERICDOMTREECONSTRUCTION_H

#include "lvc/ADT/ArrayRef.h"
#include "lvc/ADT/DenseSet.h"
#include "lvc/ADT/DepthFirstIterator.h"
#include "lvc/ADT/PointerIntPair.h"
#include "lvc/ADT/SmallPtrSet.h"
#include "lvc/Support/Debug.h"
#include "lvc/Support/GenericDomTree.h"
#include <queue>
...
struct SemiNCAInfo {
  ...
  static RootsT FindRoots(const DomTreeT &DT, BatchUpdatePtr BUI) {
    ...
    LVC_DEBUG(dbgs() << "\t\tLooking for trivial roots\n");
    ...
    for (const NodePtr N : nodes(DT.Parent)) {
      ...
      if (!HasForwardSuccessors(N, BUI)) {
        ...
        LVC_DEBUG(dbgs() << "Found a new trivial root: " << BlockNamePrinter(N)
                          << "\n");
        LVC_DEBUG(dbgs() << "Last visited node: "
                          << BlockNamePrinter(SNCA.NumToNode[Num]) << "\n");
      }
    }

    LVC_DEBUG(dbgs() << "\t\tLooking for non-trivial roots\n");
    ...
    if (Total + 1 != Num) {
      ...
      for (const NodePtr I : nodes(DT.Parent)) {
        if (SNCA.NodeToInfo.count(I) == 0) {
          ...
          LVC_DEBUG(dbgs() << "\t\t\tRunning forward DFS\n");
    ...
    LVC_DEBUG(dbgs() << "Found roots: ");
    LVC_DEBUG(for (auto *Root
                    : Roots) dbgs()
  ...
  static void RemoveRedundantRoots(const DomTreeT &DT, BatchUpdatePtr BUI,
    ...
    LVC_DEBUG(dbgs() << "Removing redundant roots\n");
    ...
    for (unsigned i = 0; i < Roots.size(); ++i) {
      ...
      LVC_DEBUG(dbgs() << "\tChecking if " << BlockNamePrinter(Root)
                        << " remains a root\n");
      SNCA.clear();
      ...
      for (unsigned x = 2; x <= Num; ++x) {
        ...
        // one.
        if (lvc::s_contained(Roots, N)) {
          LVC_DEBUG(dbgs() << "\tForward DFS walk found another root "
                            << BlockNamePrinter(N) << "\n\tRemoving root "
                            << BlockNamePrinter(Root) << "\n");
  ...
  static void InsertEdge(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    LVC_DEBUG(dbgs() << "Inserting edge " << BlockNamePrinter(From) << " -> "
                      << BlockNamePrinter(To) << "\n");
  ...
  static bool UpdateRootsBeforeInsertion(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    if (!lvc::s_contained(DT.Roots, To->getBlock()))
      return false;  // To is not a root, nothing to update.

    LVC_DEBUG(dbgs() << "\t\tAfter the insertion, " << BlockNamePrinter(To)
                      << " is no longer a root\n\t\tRebuilding the tree!!!\n");
  ...
  static void UpdateRootsAfterUpdate(DomTreeT &DT, const BatchUpdatePtr BUI) {
    ...
    if (!isPermutation(DT.Roots, Roots)) {
      ...
      LVC_DEBUG(dbgs() << "Roots are different in updated trees\n"
  ...
  static void InsertReachable(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    LVC_DEBUG(dbgs() << "\tReachable " << BlockNamePrinter(From->getBlock())
    ...
    LVC_DEBUG(dbgs() << "\t\tNCA == " << BlockNamePrinter(NCD) << "\n");
    ...
    while (!II.Bucket.empty()) {
      ...
      LVC_DEBUG(dbgs() << "Mark " << BlockNamePrinter(TN) <<
      ...
      while (true) {
        ...
        for (const NodePtr Succ : getChildren<IsPostDom>(TN->getBlock(), BUI)) {
          ...
          LVC_DEBUG(dbgs() << "\tSuccessor " << BlockNamePrinter(Succ)
                            << ", level = " << SuccLevel << "\n");
          ...
          if (SuccLevel > CurrentLevel) {
            ...
            LVC_DEBUG(dbgs() << "\t\tMarking visited not affected "
            ...
            // bucket queue.
            LVC_DEBUG(dbgs() << "\t\tAdd " << BlockNamePrinter(Succ)
                              << " to a Bucket\n");
        ...
        LVC_DEBUG(dbgs() << " Next: " << BlockNamePrinter(TN) << "\n");
      }
    }
  ...
  static void UpdateInsertion(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    LVC_DEBUG(dbgs() << "Updating NCD = " << BlockNamePrinter(NCD) << "\n");

    for (const TreeNodePtr TN : II.Affected) {
      LVC_DEBUG(dbgs() << "\tIDom(" << BlockNamePrinter(TN)
  ...
  static void InsertUnreachable(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    LVC_DEBUG(dbgs() << "Inserting " << BlockNamePrinter(From)
    ...
    LVC_DEBUG(dbgs() << "Inserted " << BlockNamePrinter(From)
    ...
    for (const auto &Edge : DiscoveredEdgesToReachable) {
      LVC_DEBUG(dbgs() << "\tInserting discovered connecting edge "
  ...
  static void ComputeUnreachableDominators(
    ...
    LVC_DEBUG(dbgs() << "After adding unreachable nodes\n");
  }

  static void DeleteEdge(DomTreeT &DT, const BatchUpdatePtr BUI,
                         const NodePtr From, const NodePtr To) {
    assert(From && To && "Cannot disconnect nullptrs");
    LVC_DEBUG(dbgs() << "Deleting edge " << BlockNamePrinter(From) << " -> "
                      << BlockNamePrinter(To) << "\n");
    ...
    auto IsSuccessor = [BUI](const NodePtr SuccCandidate, const NodePtr Of) {
      ...
      return lvc::s_contained(Successors, SuccCandidate);
    };
    (void)IsSuccessor;
    ...
    if (ToTN != NCD) {
      ...
      LVC_DEBUG(dbgs() << "\tNCD " << BlockNamePrinter(NCD) << ", ToIDom "
  ...
  static void DeleteReachable(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    LVC_DEBUG(dbgs() << "Deleting reachable " << BlockNamePrinter(FromTN)
                      << " -> " << BlockNamePrinter(ToTN) << "\n");
    LVC_DEBUG(dbgs() << "\tRebuilding subtree\n");
    ...
    // scratch.
    if (!PrevIDomSubTree) {
      LVC_DEBUG(dbgs() << "The entire tree needs to be rebuilt\n");
      CalculateFromScratch(DT, BUI);
      return;
    }
    ...
    LVC_DEBUG(dbgs() << "\tTop of subtree: " << BlockNamePrinter(ToIDomTN)
                      << "\n");

    SemiNCAInfo SNCA(BUI);
    SNCA.runDFS(ToIDom, 0, DescendBelow, 0);
    LVC_DEBUG(dbgs() << "\tRunning Semi-NCA\n");
  ...
  static bool HasProperSupport(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    LVC_DEBUG(dbgs() << "IsReachableFromIDom " << BlockNamePrinter(TN)
                      << "\n");
    auto TNB = TN->getBlock();
    for (const NodePtr Pred : getChildren<!IsPostDom>(TNB, BUI)) {
      LVC_DEBUG(dbgs() << "\tPred " << BlockNamePrinter(Pred) << "\n");
      if (!DT.getNode(Pred)) continue;

      const NodePtr Support = DT.findNearestCommonDominator(TNB, Pred);
      LVC_DEBUG(dbgs() << "\tSupport " << BlockNamePrinter(Support) << "\n");
      if (Support != TNB) {
        LVC_DEBUG(dbgs() << "\t" << BlockNamePrinter(TN)
  ...
  static void DeleteUnreachable(DomTreeT &DT, const BatchUpdatePtr BUI,
                                const TreeNodePtr ToTN) {
    LVC_DEBUG(dbgs() << "Deleting unreachable subtree "
                      << BlockNamePrinter(ToTN) << "\n");
    assert(ToTN);
    ...
    if (IsPostDom) {
      ...
      LVC_DEBUG(dbgs() << "\tDeletion made a region reverse-unreachable\n");
      LVC_DEBUG(dbgs() << "\tAdding new root " << BlockNamePrinter(ToTN)
                        << "\n");
    ...
    auto DescendAndCollect = [Level, &AffectedQueue, &DT](NodePtr, NodePtr To) {
      ...
      if (!lvc::s_contained(AffectedQueue, To))
    ...
    for (const NodePtr N : AffectedQueue) {
      ...
      LVC_DEBUG(dbgs() << "Processing affected node " << BlockNamePrinter(TN)
    ...
    for (unsigned i = LastDFSNum; i > 0; --i) {
      const NodePtr N = SNCA.NumToNode[i];
      const TreeNodePtr TN = DT.getNode(N);
      LVC_DEBUG(dbgs() << "Erasing node " << BlockNamePrinter(TN) << "\n");
    ...
    LVC_DEBUG(dbgs() << "DeleteUnreachable: running DFS with MinNode = "
    ...
    LVC_DEBUG(dbgs() << "Previous IDom(MinNode) = "
  ...
  static void EraseNode(DomTreeT &DT, const TreeNodePtr TN) {
    ...
    auto ChIt = lvc::ind(IDom->Children, TN);
...
#if 0
    // FIXME: The LVC_DEBUG macro only plays well with a modular
    // build of LLVM when the header is marked as textual, but doing
    // so causes redefinition errors.
    LVC_DEBUG(dbgs() << "Applying update: ");
    LVC_DEBUG(CurrentUpdate.dump(); dbgs() << "\n");
#endif
  ...
  static bool VerifyDFSNumbers(const DomTreeT &DT) {
    ...
    for (const auto &NodeToTN : DT.DomTreeNodes) {
      ...
      lvc::ort(Children, [](const TreeNodePtr Ch1, const TreeNodePtr Ch2) {
  ...
  bool verifyParentProperty(const DomTreeT &DT) {
    for (auto &NodeToTN : DT.DomTreeNodes) {
      ...
      LVC_DEBUG(dbgs() << "Verifying parent property of node "
                        << BlockNamePrinter(TN) << "\n");
      clear();
