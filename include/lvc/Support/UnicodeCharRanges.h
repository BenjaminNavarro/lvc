...
//
//===----------------------------------------------------------------------===//
#ifndef LVC_SUPPORT_UNICODECHARRANGES_H
#define LVC_SUPPORT_UNICODECHARRANGES_H
  ...
  bool rangesAreValid() const {
    ...
    for (CharRanges::const_iterator I = Ranges.begin(), E = Ranges.end();
      ...
      if (I != Ranges.begin() && Prev >= I->Lower) {
        LVC_DEBUG(dbgs() << "Upper bound 0x");
        LVC_DEBUG(dbgs().write_hex(Prev));
        LVC_DEBUG(dbgs() << " should be less than succeeding lower bound 0x");
        LVC_DEBUG(dbgs().write_hex(I->Lower) << "\n");
        return false;
      }
      if (I->Upper < I->Lower) {
        LVC_DEBUG(dbgs() << "Upper bound 0x");
        LVC_DEBUG(dbgs().write_hex(I->Lower));
        LVC_DEBUG(dbgs() << " should not be less than lower bound 0x");
        LVC_DEBUG(dbgs().write_hex(I->Upper) << "\n");
        return false;
      }
      Prev = I->Upper;
    }
...
#endif // LVC_SUPPORT_UNICODECHARRANGES_H
