...
#ifndef LVC_SUPPORT_CASTING_H
#define LVC_SUPPORT_CASTING_H
...
//
template <class X, class Y> LVC_NODISCARD inline bool isa(const Y &Val) {
  return isa_impl_wrap<X, const Y,
                       typename simplify_type<const Y>::SimpleType>::doit(Val);
}

template <typename First, typename Second, typename... Rest, typename Y>
LVC_NODISCARD inline bool isa(const Y &Val) {
...
LVC_NODISCARD inline bool isa_and_nonnull(const Y &Val) {
  if (!Val)
    return false;
  return isa<X...>(Val);
}
...
//
template <class X, class Y>
LVC_NODISCARD inline std::enable_if_t<
...
template <class X, class Y>
LVC_NODISCARD inline std::enable_if_t<!is_simple_type<Y>::value,
...
template <class X, class Y>
LVC_NODISCARD inline typename cast_retty<X, Y *>::ret_type
cast_or_null(Y *Val) {
...
template <class X, class Y>
LVC_NODISCARD inline typename cast_retty<X, Y>::ret_type dyn_cast(Y &Val) {
  return isa<X>(Val) ? cast<X>(Val) : nullptr;
}
...
template <class X, class Y>
LVC_NODISCARD inline auto unique_dyn_cast(std::unique_ptr<Y> &Val)
    -> decltype(cast<X>(Val)) {
  if (!isa<X>(Val))
    return nullptr;
  return cast<X>(std::move(Val));
...
template <class X, class Y>
LVC_NODISCARD inline auto unique_dyn_cast_or_null(std::unique_ptr<Y> &Val)
    -> decltype(cast<X>(Val)) {
  if (!Val)
    return nullptr;
  return unique_dyn_cast<X, Y>(Val);
