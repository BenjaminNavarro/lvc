...
#ifndef LVC_SUPPORT_CHRONO_H
#define LVC_SUPPORT_CHRONO_H
...
template <> struct format_provider<sys::TimePoint<>> {
  static void format(const sys::TimePoint<> &TP, lvc::raw_ostream &OS,
                     StringRef Style);
};
...
public:
  static void format(const Dur &D, lvc::raw_ostream &Stream, StringRef Style) {
    InternalRep count;
    StringRef unit;
