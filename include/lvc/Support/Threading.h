...
#ifndef LVC_SUPPORT_THREADING_H
#define LVC_SUPPORT_THREADING_H

#include "lvc/ADT/BitVector.h"
#include "lvc/ADT/FunctionExtras.h"
#include "lvc/ADT/SmallVector.h"
#include "lvc/Config/llvm-config.h" // for LVC_ON_UNIX
...
#define LVC_THREADING_USE_STD_CALL_ONCE 1
...
#define LVC_THREADING_USE_STD_CALL_ONCE 1
#else
#define LVC_THREADING_USE_STD_CALL_ONCE 0
#endif

#if LVC_THREADING_USE_STD_CALL_ONCE
#include <mutex>
#else
#include "lvc/Support/Atomic.h"
#endif
...
/// (or None for default)
void LVC_execute_on_thread(
    void (*UserFn)(void *), void *UserData,
    lvc::Optional<unsigned> StackSizeInBytes = lvc::None);
...
/// \param StackSizeInBytes - A requested size (in bytes) for the thread stack
/// (or None for default)
void LVC_execute_on_thread_async(
...
/// The lvc::once_flag structure
///
/// This type is modeled after std::once_flag to use with lvc::call_once.
...
/// configured with LVC_ENABLE_THREADS=OFF.
...
lvc::BitVector get_thread_affinity_mask();
