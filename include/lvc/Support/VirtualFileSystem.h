...
#ifndef LVC_SUPPORT_VIRTUALFILESYSTEM_H
#define LVC_SUPPORT_VIRTUALFILESYSTEM_H
...
class Status {
  std::string Name;
  lvc::sys::fs::UniqueID UID;
  lvc::sys::TimePoint<> MTime;
  uint32_t User;
  uint32_t Group;
  uint64_t Size;
  lvc::sys::fs::file_type Type = lvc::sys::fs::file_type::status_error;
  lvc::sys::fs::perms Perms;
...
public:
  ...
  Status() = default;
  Status(const lvc::sys::fs::file_status &Status);
  Status(const Twine &Name, lvc::sys::fs::UniqueID UID,
         lvc::sys::TimePoint<> MTime, uint32_t User, uint32_t Group,
         uint64_t Size, lvc::sys::fs::file_type Type,
         lvc::sys::fs::perms Perms);

  /// Get a copy of a Status with a different name.
  static Status copyWithNewName(const Status &In, const Twine &NewName);
  static Status copyWithNewName(const lvc::sys::fs::file_status &In,
                                const Twine &NewName);

  /// Returns the name that should be used for this file or directory.
  StringRef getName() const { return Name; }

  /// @name Status interface from lvc::sys::fs
  /// @{
  lvc::sys::fs::file_type getType() const { return Type; }
  lvc::sys::fs::perms getPermissions() const { return Perms; }
  lvc::sys::TimePoint<> getLastModificationTime() const { return MTime; }
  lvc::sys::fs::UniqueID getUniqueID() const { return UID; }
  uint32_t getUser() const { return User; }
  uint32_t getGroup() const { return Group; }
  uint64_t getSize() const { return Size; }
  /// @}
  /// @name Status queries
  /// These are static queries in lvc::sys::fs.
  /// @{
...
public:
  ...
  virtual lvc::ErrorOr<Status> status() = 0;

  /// Get the name of the file
  virtual lvc::ErrorOr<std::string> getName() {
  ...
  virtual lvc::ErrorOr<std::unique_ptr<lvc::MemoryBuffer>>
...
class directory_entry {
  std::string Path;
  lvc::sys::fs::file_type Type = lvc::sys::fs::file_type::type_unknown;

public:
  directory_entry() = default;
  directory_entry(std::string Path, lvc::sys::fs::file_type Type)
      : Path(std::move(Path)), Type(Type) {}

  lvc::StringRef path() const { return Path; }
  lvc::sys::fs::file_type type() const { return Type; }
};
...
/// similar to lvc::sys::fs::recursive_directory_iterator.
...
class FileSystem : public lvc::ThreadSafeRefCountedBase<FileSystem> {
public:
  virtual ~FileSystem();

  /// Get the status of the entry at \p Path, if one exists.
  virtual lvc::ErrorOr<Status> status(const Twine &Path) = 0;

  /// Get a \p File object for the file at \p Path, if one exists.
  virtual lvc::ErrorOr<std::unique_ptr<File>>
  ...
  virtual lvc::ErrorOr<std::string> getCurrentWorkingDirectory() const = 0;

  /// Gets real path of \p Path e.g. collapse all . and .. patterns, resolve
  /// symlinks. For real file system, this uses `lvc::sys::fs::real_path`.
...
public:
  ...
  lvc::ErrorOr<Status> status(const Twine &Path) override;
...
public:
  ...
  bool addFile(const Twine &Path, time_t ModificationTime,
               ...
               Optional<lvc::sys::fs::file_type> Type = None,
               Optional<lvc::sys::fs::perms> Perms = None);
...
lvc::sys::fs::UniqueID getNextVirtualUniqueID();

/// Gets a \p FileSystem for a virtual file system described in YAML
/// format.
std::unique_ptr<FileSystem>
getVFSFromYAML(std::unique_ptr<lvc::MemoryBuffer> Buffer,
               lvc::SourceMgr::DiagHandlerTy DiagHandler,
...
public:
  ...
  public:
    ...
    static bool classof(const Entry *E) {
      ...
      lvc_unreachable("invalid entry kind");
    }
  };
...
private:
  ...
  // backslashes.
  bool pathComponentMatches(lvc::StringRef lhs, lvc::StringRef rhs) const {
  ...
  ErrorOr<LookupResult> lookupPathImpl(lvc::sys::path::const_iterator Start,
                                       lvc::sys::path::const_iterator End,
                                       Entry *From) const;
...
void collectVFSFromYAML(
    std::unique_ptr<lvc::MemoryBuffer> Buffer,
    lvc::SourceMgr::DiagHandlerTy DiagHandler, StringRef YAMLFilePath,
...
public:
  ...
  void write(lvc::raw_ostream &OS);
};

} // namespace vfs
} // namespace lvc

#endif // LVC_SUPPORT_VIRTUALFILESYSTEM_H
