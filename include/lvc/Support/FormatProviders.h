...
#ifndef LVC_SUPPORT_FORMATPROVIDERS_H
#define LVC_SUPPORT_FORMATPROVIDERS_H

#include "lvc/ADT/Optional.h"
#include "lvc/ADT/STLExtras.h"
#include "lvc/ADT/StringSwitch.h"
#include "lvc/ADT/Twine.h"
#include "lvc/Support/FormatVariadicDetails.h"
...
struct use_string_formatter
    : public std::integral_constant<
          bool, std::is_convertible<T, lvc::StringRef>::value> {};
...
private:
public:
  static void format(const T &V, lvc::raw_ostream &Stream, StringRef Style) {
    HexPrintStyle HS;
    size_t Digits = 0;
...
/// objects such as std::string and lvc::StringRef.
///
...
/// Implementation of format_provider<T> for lvc::Twine.
///
/// This follows the same rules as the string formatter.

template <> struct format_provider<Twine> {
  static void format(const Twine &V, lvc::raw_ostream &Stream,
                     StringRef Style) {
...
struct format_provider<T,
  ...
  static void format(const char &V, lvc::raw_ostream &Stream, StringRef Style) {
    if (Style.empty())
      Stream << V;
    else {
...
template <> struct format_provider<bool> {
  static void format(const bool &B, lvc::raw_ostream &Stream, StringRef Style) {
...
template <typename IterT> class format_provider<lvc::iterator_range<IterT>> {
...
public:
  ...
  static void format(const lvc::iterator_range<IterT> &V,
                     lvc::raw_ostream &Stream, StringRef Style) {
    StringRef Sep;
    StringRef ArgStyle;
