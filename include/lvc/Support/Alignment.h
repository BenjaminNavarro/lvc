...
#ifndef LVC_SUPPORT_ALIGNMENT_H_
#define LVC_SUPPORT_ALIGNMENT_H_
...
public:
  ...
  explicit Align(uint64_t Value) {
    ...
    assert(lvc::isPowerOf2_64(Value) && "Alignment is not a power of 2");
  ...
  /// lvc::None which can be used to initialize `MaybeAlign`.
  /// MaybeAlign = lvc::None means unspecified alignment,
  /// Align = Align::None() means alignment of one byte.
  LVC_ATTRIBUTE_DEPRECATED(constexpr static const Align None(),
...
struct MaybeAlign : public lvc::Optional<Align> {
private:
  using UP = lvc::Optional<Align>;
...
public:
  ...
  /// Use lvc::Optional<Align> constructor.
  using UP::UP;

  explicit MaybeAlign(uint64_t Value) {
    assert((Value == 0 || lvc::isPowerOf2_64(Value)) &&
...
inline Align operator/(Align Lhs, uint64_t Divisor) {
  assert(lvc::isPowerOf2_64(Divisor) &&
...
#endif // LVC_SUPPORT_ALIGNMENT_H_
