...
#ifndef LVC_SUPPORT_FORMATVARIADICDETAILS_H
#define LVC_SUPPORT_FORMATVARIADICDETAILS_H
...
public:
  ...
  void format(lvc::raw_ostream &S, StringRef Options) override {
...
public:
  ...
  void format(lvc::raw_ostream &S, StringRef) override { S << Item; }
};
...
public:
  ...
  typedef void (*Signature_format)(const Decayed &, lvc::raw_ostream &,
                                   StringRef);
  ...
  static bool const value =
      (sizeof(test<lvc::format_provider<Decayed>>(nullptr)) == 1);
};
...
public:
  ...
  template <typename U>
  static char test(
      std::enable_if_t<std::is_same<decltype(std::declval<lvc::raw_ostream &>()
                                             << std::declval<U>()),
                                    lvc::raw_ostream &>::value,
                       int *>);
...
build_format_adapter(T &&Item) {
  ...
  static_assert(
      !std::is_same<lvc::Error, std::remove_cv_t<T>>::value,
      "lvc::Error-by-value must be wrapped in fmt_consume() for formatv");
