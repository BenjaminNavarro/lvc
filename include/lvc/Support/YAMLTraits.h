...
#ifndef LVC_SUPPORT_YAMLTRAITS_H
#define LVC_SUPPORT_YAMLTRAITS_H
...
#include "lvc/Support/AlignOf.h"
#include "lvc/Support/Allocator.h"
#include "lvc/Support/Endian.h"
#include "lvc/Support/Regex.h"
...
///      static void output(const MyType &val, void*, lvc::raw_ostream &out) {
///        // stream out custom formatting
///        out << lvc::format("%x", val);
///      }
...
template <typename T, typename Enable = void> struct ScalarTraits {
  ...
  // static void output(const T &value, void *ctxt, lvc::raw_ostream &out);
  //
...
///      static void output(const MyType &Value, void*, lvc::raw_ostream &Out)
///      {
///        // stream out custom formatting
///        Out << Value;
///      }
...
template <typename T> struct BlockScalarTraits {
  ...
  //
  // Function to write the value as a string:
  // static void output(const T &Value, void *ctx, lvc::raw_ostream &Out);
  //
  // Function to convert a string to a value.  Returns the empty
  // StringRef on success or an error string if string is malformed:
  // static StringRef input(StringRef Scalar, void *ctxt, T &Value);
...
///      static void output(const MyType &Value, void*, lvc::raw_ostream
///      &ScalarOut, lvc::raw_ostream &TagOut)
///      {
...
template <typename T> struct TaggedScalarTraits {
  ...
  // static void output(const T &Value, void *ctx, lvc::raw_ostream &ScalarOut,
  // lvc::raw_ostream &TagOut);
  //
  // Function to convert a string to a value.  Returns the empty
  // StringRef on success or an error string if string is malformed:
...
public:
  ...
  // allow anonymous enum values to be used with LVC_YAML_STRONG_TYPEDEF
  template <typename T>
...
private:
  ...
  std::unique_ptr<lvc::yaml::Stream> Strm;
...
#define LVC_YAML_STRONG_TYPEDEF(_base, _type)                                  \
  struct _type {                                                               \
    _type() = default;                                                         \
...
///
LVC_YAML_STRONG_TYPEDEF(uint8_t, Hex8)
LVC_YAML_STRONG_TYPEDEF(uint16_t, Hex16)
LVC_YAML_STRONG_TYPEDEF(uint32_t, Hex32)
LVC_YAML_STRONG_TYPEDEF(uint64_t, Hex64)
...
template <> struct ScalarTraits<VersionTuple> {
  static void output(const VersionTuple &Value, void *, lvc::raw_ostream &Out);
...
#define LVC_YAML_IS_SEQUENCE_VECTOR_IMPL(TYPE, FLOW)                           \
  namespace lvc {                                                              \
  namespace yaml {                                                             \
  static_assert(!std::is_fundamental<TYPE>::value &&                           \
                    !std::is_same<TYPE, std::string>::value &&                 \
                    !std::is_same<TYPE, lvc::StringRef>::value,                \
                "only use LVC_YAML_IS_SEQUENCE_VECTOR for types you control"); \
...
#define LVC_YAML_IS_SEQUENCE_VECTOR(type)                                      \
  LVC_YAML_IS_SEQUENCE_VECTOR_IMPL(type, false)

/// Utility for declaring that a std::vector of a particular type
/// should be considered a YAML flow sequence.
#define LVC_YAML_IS_FLOW_SEQUENCE_VECTOR(type)                                 \
  LVC_YAML_IS_SEQUENCE_VECTOR_IMPL(type, true)

#define LVC_YAML_DECLARE_MAPPING_TRAITS(Type)                                  \
  namespace lvc {                                                              \
  namespace yaml {                                                             \
...
#define LVC_YAML_DECLARE_ENUM_TRAITS(Type)                                     \
  namespace lvc {                                                              \
  namespace yaml {                                                             \
...
#define LVC_YAML_DECLARE_BITSET_TRAITS(Type)                                   \
  namespace lvc {                                                              \
  namespace yaml {                                                             \
...
#define LVC_YAML_DECLARE_SCALAR_TRAITS(Type, MustQuote)                        \
  namespace lvc {                                                              \
  namespace yaml {                                                             \
...
#define LVC_YAML_IS_DOCUMENT_LIST_VECTOR(_type)                                \
  namespace lvc {                                                              \
  namespace yaml {                                                             \
  template <unsigned N>                                                        \
...
/// a YAML map.
#define LVC_YAML_IS_STRING_MAP(_type)                                          \
  namespace lvc {                                                              \
  namespace yaml {                                                             \
  template <>                                                                  \
...
LVC_YAML_IS_FLOW_SEQUENCE_VECTOR(lvc::yaml::Hex64)
LVC_YAML_IS_FLOW_SEQUENCE_VECTOR(lvc::yaml::Hex32)
LVC_YAML_IS_FLOW_SEQUENCE_VECTOR(lvc::yaml::Hex16)
LVC_YAML_IS_FLOW_SEQUENCE_VECTOR(lvc::yaml::Hex8)

#endif // LVC_SUPPORT_YAMLTRAITS_H
