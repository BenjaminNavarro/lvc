...
// fun:sin=functional
// ---
// Note that the wild card is in fact an lvc::Regex, but * is automatically
// replaced with .*
//
//===----------------------------------------------------------------------===//

#ifndef LVC_SUPPORT_SPECIALCASELIST_H
#define LVC_SUPPORT_SPECIALCASELIST_H

#include "lvc/ADT/StringMap.h"
#include "lvc/Support/Regex.h"
#include "lvc/Support/TrigramIndex.h"
#include <memory>
#include <string>
#include <vector>
...
public:
  ...
  create(const std::vector<std::string> &Paths, lvc::vfs::FileSystem &FS,
         std::string &Error);
  ...
  /// fatal error.
  static std::unique_ptr<SpecialCaseList>
  createOrDie(const std::vector<std::string> &Paths, lvc::vfs::FileSystem &FS);
...
#endif // LVC_SUPPORT_SPECIALCASELIST_H
