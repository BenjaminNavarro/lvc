...
#ifndef LVC_SUPPORT_JSON_H
#define LVC_SUPPORT_JSON_H
...
bool isUTF8(lvc::StringRef S, size_t *ErrOffset = nullptr);
...
std::string fixUTF8(lvc::StringRef S);

class Array;
class ObjectKey;
class Value;
template <typename T> Value toJSON(const lvc::Optional<T> &Opt);

/// An Object is a JSON object, which maps strings to heterogenous JSON values.
/// It simulates DenseMap<ObjectKey, Value>. ObjectKey is a maybe-owned string.
class Object {
  using Storage = DenseMap<ObjectKey, Value, lvc::DenseMapInfo<StringRef>>;
  Storage M;
...
public:
  ...
  lvc::Optional<std::nullptr_t> getNull(StringRef K) const;
  lvc::Optional<bool> getBoolean(StringRef K) const;
  lvc::Optional<double> getNumber(StringRef K) const;
  lvc::Optional<int64_t> getInteger(StringRef K) const;
  lvc::Optional<lvc::StringRef> getString(StringRef K) const;
...
public:
  ...
  Kind kind() const {
    ...
    lvc_unreachable("Unknown kind");
  }

  // Typed accessors return None/nullptr if the Value is not of this type.
  lvc::Optional<std::nullptr_t> getAsNull() const {
    if (LVC_LIKELY(Type == T_Null))
      return nullptr;
    return lvc::None;
  }
  lvc::Optional<bool> getAsBoolean() const {
    if (LVC_LIKELY(Type == T_Boolean))
      return as<bool>();
    return lvc::None;
  }
  lvc::Optional<double> getAsNumber() const {
    if (LVC_LIKELY(Type == T_Double))
      return as<double>();
    if (LVC_LIKELY(Type == T_Integer))
      return as<int64_t>();
    return lvc::None;
  }
  // Succeeds if the Value is a Number, and exactly representable as int64_t.
  lvc::Optional<int64_t> getAsInteger() const {
    if (LVC_LIKELY(Type == T_Integer))
      return as<int64_t>();
    if (LVC_LIKELY(Type == T_Double)) {
      double D = as<double>();
      if (LVC_LIKELY(std::modf(D, &D) == 0.0 &&
  ...
  lvc::Optional<lvc::StringRef> getAsString() const {
    if (Type == T_String)
      return lvc::StringRef(as<std::string>());
    if (LVC_LIKELY(Type == T_StringRef))
      return as<lvc::StringRef>();
    return lvc::None;
  }
  const json::Object *getAsObject() const {
    return LVC_LIKELY(Type == T_Object) ? &as<json::Object>() : nullptr;
  }
  json::Object *getAsObject() {
    return LVC_LIKELY(Type == T_Object) ? &as<json::Object>() : nullptr;
  }
  const json::Array *getAsArray() const {
    return LVC_LIKELY(Type == T_Array) ? &as<json::Array>() : nullptr;
  }
...
private:
  ...
  mutable lvc::AlignedCharArrayUnion<bool, double, int64_t, lvc::StringRef,
...
public:
  ...
  ObjectKey(std::string S) : Owned(new std::string(std::move(S))) {
    if (LVC_UNLIKELY(!isUTF8(*Owned))) {
      assert(false && "Invalid UTF-8 in value used as JSON");
      *Owned = fixUTF8(std::move(*Owned));
    }
    Data = *Owned;
  }
  ObjectKey(lvc::StringRef S) : Data(S) {
    if (LVC_UNLIKELY(!isUTF8(Data))) {
      assert(false && "Invalid UTF-8 in value used as JSON");
      *this = ObjectKey(fixUTF8(S));
    }
  }
  ObjectKey(const lvc::SmallVectorImpl<char> &V)
      : ObjectKey(std::string(V.begin(), V.end())) {}
  ObjectKey(const lvc::formatv_object_base &V) : ObjectKey(V.str()) {}
  ...
  operator lvc::StringRef() const { return Data; }
...
inline bool operator==(const ObjectKey &L, const ObjectKey &R) {
  return lvc::StringRef(L) == lvc::StringRef(R);
}
inline bool operator!=(const ObjectKey &L, const ObjectKey &R) {
  return !(L == R);
}
...
public:
  ...
  void report(lvc::StringLiteral Message);
...
class Path::Root {
  lvc::StringRef Name;
  lvc::StringLiteral ErrorMessage;
  std::vector<Path::Segment> ErrorPath; // Only valid in error state. Reversed.

  friend void Path::report(lvc::StringLiteral Message);

public:
  Root(lvc::StringRef Name = "") : Name(Name), ErrorMessage("") {}
  ...
  ///      "properties": { ... }
  ///   }
  void printErrorContext(const Value &, lvc::raw_ostream &) const;
};
...
template <typename T>
bool fromJSON(const Value &E, lvc::Optional<T> &Out, Path P) {
  if (E.getAsNull()) {
    Out = lvc::None;
    return true;
  }
...
bool fromJSON(const Value &E, std::map<std::string, T> &Out, Path P) {
  if (auto *O = E.getAsObject()) {
    Out.clear();
    for (const auto &KV : *O)
      if (!fromJSON(KV.second, Out[std::string(lvc::StringRef(KV.first))],
                    P.field(KV.first)))
        return false;
    return true;
  }
...
public:
  ...
  template <typename T> bool map(StringLiteral Prop, lvc::Optional<T> &Out) {
    assert(*this && "Must check this is an object before calling map()");
    if (const Value *E = O->get(Prop))
      return fromJSON(*E, Out, P.field(Prop));
    Out = lvc::None;
...
lvc::Expected<Value> parse(lvc::StringRef JSON);

class ParseError : public lvc::ErrorInfo<ParseError> {
  const char *Msg;
...
public:
  ...
  void log(lvc::raw_ostream &OS) const override {
    OS << lvc::formatv("[{0}:{1}, byte={2}]: {3}", Line, Column, Offset, Msg);
  }
  std::error_code convertToErrorCode() const override {
    return lvc::inconvertibleErrorCode();
  }
};
...
template <typename T>
Expected<T> parse(const lvc::StringRef &JSON, const char *RootName = "") {
  auto V = parse(JSON);
  if (!V)
    return V.takeError();
...
class OStream {
public:
  using Block = lvc::function_ref<void()>;
  // If IndentSize is nonzero, output is pretty-printed.
  explicit OStream(lvc::raw_ostream &OS, unsigned IndentSize = 0)
  ...
  void rawValue(lvc::function_ref<void(raw_ostream &)> Contents) {
    rawValueBegin();
    Contents(OS);
    rawValueEnd();
  }
  void rawValue(lvc::StringRef Contents) {
  ...
  void objectBegin();
  void objectEnd();
  void attributeBegin(lvc::StringRef Key);
  void attributeEnd();
  raw_ostream &rawValueBegin();
  void rawValueEnd();

private:
  void attributeImpl(lvc::StringRef Key, Block Contents) {
    attributeBegin(Key);
    Contents();
    attributeEnd();
  }
  ...
  lvc::SmallVector<State, 16> Stack; // Never empty.
  lvc::StringRef PendingComment;
  lvc::raw_ostream &OS;
  unsigned IndentSize;
  unsigned Indent = 0;
};
...
inline lvc::raw_ostream &operator<<(lvc::raw_ostream &OS, const Value &V) {
  OStream(OS).value(V);
  return OS;
}
} // namespace json
...
template <> struct format_provider<lvc::json::Value> {
  static void format(const lvc::json::Value &, raw_ostream &, StringRef);
};
} // namespace lvc
