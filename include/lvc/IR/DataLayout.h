...
#ifndef LVC_IR_DATALAYOUT_H
#define LVC_IR_DATALAYOUT_H
...
#include "lvc/Support/TrailingObjects.h"
#include "lvc/Support/TypeSize.h"
#include <cassert>
#include <cstdint>
#include <string>
...
public:
  ...
  bool isLegalInteger(uint64_t Width) const {
    return lvc::is_contained(LegalIntWidths, Width);
  }
  ...
  char getGlobalPrefix() const {
    ...
    lvc_unreachable("invalid mangling mode");
  }
...
public:
  ...
  MutableArrayRef<uint64_t> getMemberOffsets() {
    return lvc::makeMutableArrayRef(getTrailingObjects<uint64_t>(),
                                     NumElements);
  }

  ArrayRef<uint64_t> getMemberOffsets() const {
    return lvc::makeArrayRef(getTrailingObjects<uint64_t>(), NumElements);
  }
...
inline TypeSize DataLayout::getTypeSizeInBits(Type *Ty) const {
  ...
  default:
    lvc_unreachable("DataLayout::getTypeSizeInBits(): Unsupported type");
  }
}

} // end namespace lvc

#endif // LVC_IR_DATALAYOUT_H
