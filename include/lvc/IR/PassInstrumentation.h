...
///
///    - PassInstrumentation wraps address of IRUnit into lvc::Any and passes
...
#ifndef LVC_IR_PASSINSTRUMENTATION_H
#define LVC_IR_PASSINSTRUMENTATION_H

#include "lvc/ADT/Any.h"
#include "lvc/ADT/FunctionExtras.h"
...
public:
  ...
  // to take them as constant pointers, wrapped with lvc::Any.
...
public:
  ...
  bool runBeforePass(const PassT &Pass, const IRUnitT &IR) const {
    ...
    if (!isRequired(Pass)) {
      for (auto &C : Callbacks->ShouldRunOptionalPassCallbacks)
        ShouldRun &= C(Pass.name(), lvc::Any(&IR));
    }

    if (ShouldRun) {
      for (auto &C : Callbacks->BeforeNonSkippedPassCallbacks)
        C(Pass.name(), lvc::Any(&IR));
    } else {
  ...
  bool invalidate(IRUnitT &, const class lvc::PreservedAnalyses &,
                  ExtraArgsT...) {
    return false;
  }
