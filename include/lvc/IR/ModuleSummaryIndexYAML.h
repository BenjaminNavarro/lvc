...
#ifndef LVC_IR_MODULESUMMARYINDEXYAML_H
#define LVC_IR_MODULESUMMARYINDEXYAML_H

#include "lvc/IR/ModuleSummaryIndex.h"
...
struct CustomMappingTraits<std::map<uint64_t, WholeProgramDevirtResolution>> {
  ...
  static void output(IO &io, std::map<uint64_t, WholeProgramDevirtResolution> &V) {
    for (auto &P : V)
      io.mapRequired(lvc::utostr(P.first).c_str(), P.second);
  }
};
...
LVC_YAML_IS_SEQUENCE_VECTOR(FunctionSummary::VFuncId)
LVC_YAML_IS_SEQUENCE_VECTOR(FunctionSummary::ConstVCall)
...
LVC_YAML_IS_SEQUENCE_VECTOR(FunctionSummaryYaml)
...
template <> struct CustomMappingTraits<GlobalValueSummaryMapTy> {
  ...
  static void output(IO &io, GlobalValueSummaryMapTy &V) {
    for (auto &P : V) {
      ...
      if (!FSums.empty())
        io.mapRequired(lvc::utostr(P.first).c_str(), FSums);
    }
  }
};
