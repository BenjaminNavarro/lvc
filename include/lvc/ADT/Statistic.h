...
// end of a run (from LVC_shutdown), when the -stats command line option is
...
#ifndef LVC_ADT_STATISTIC_H
#define LVC_ADT_STATISTIC_H
...
// configure time.
#if !defined(NDEBUG) || LVC_FORCE_ENABLE_STATS
#define LVC_ENABLE_STATS 1
#else
#define LVC_ENABLE_STATS 0
#endif
...
#define STATISTIC(VARNAME, DESC)                                               \
  static lvc::Statistic VARNAME = {DEBUG_TYPE, #VARNAME, DESC}

// ALWAYS_ENABLED_STATISTIC - A macro to define a statistic like STATISTIC but
// it is enabled even if LVC_ENABLE_STATS is off.
#define ALWAYS_ENABLED_STATISTIC(VARNAME, DESC)                                \
  static lvc::TrackingStatistic VARNAME = {DEBUG_TYPE, #VARNAME, DESC}
