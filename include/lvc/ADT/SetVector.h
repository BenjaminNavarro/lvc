...
#ifndef LVC_ADT_SETVECTOR_H
#define LVC_ADT_SETVECTOR_H
...
public:
  ...
  template <typename UnaryPredicate> bool remove_if(UnaryPredicate P) {
    typename vector_type::iterator I =
        lvc::remove_if(vector_, TestAndEraseFromSet<UnaryPredicate>(P, set_));
    if (I == vector_.end())
      return false;
...
inline void swap(lvc::SetVector<T, V, S> &LHS, lvc::SetVector<T, V, S> &RHS) {
  LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallSetVector swap.
template <typename T, unsigned N>
inline void swap(lvc::SmallSetVector<T, N> &LHS,
                 lvc::SmallSetVector<T, N> &RHS) {
  LHS.swap(RHS);
}

} // end namespace std

#endif // LVC_ADT_SETVECTOR_H
