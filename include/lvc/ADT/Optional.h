...
#ifndef LVC_ADT_OPTIONAL_H
#define LVC_ADT_OPTIONAL_H

#include "lvc/ADT/Hashing.h"
#include "lvc/ADT/None.h"
#include "lvc/Support/Compiler.h"
#include "lvc/Support/type_traits.h"
#include <cassert>
#include <memory>
#include <new>
#include <utility>
...
// lvc::is_trivially_copy_constructible instead of
...
template <typename T, bool = (lvc::is_trivially_copy_constructible<T>::value &&
...
public:
  ...
  T &getValue() LVC_LVALUE_FUNCTION noexcept {
    assert(hasVal);
    return value;
  }
  constexpr T const &getValue() const LVC_LVALUE_FUNCTION noexcept {
    assert(hasVal);
    return value;
  }
...
public:
  ...
  constexpr const T &getValue() const LVC_LVALUE_FUNCTION {
    return Storage.getValue();
  }
  T &getValue() LVC_LVALUE_FUNCTION { return Storage.getValue(); }

  constexpr explicit operator bool() const { return hasValue(); }
  constexpr bool hasValue() const { return Storage.hasValue(); }
  constexpr const T *operator->() const { return getPointer(); }
  T *operator->() { return getPointer(); }
  constexpr const T &operator*() const LVC_LVALUE_FUNCTION {
    return getValue();
  }
  T &operator*() LVC_LVALUE_FUNCTION { return getValue(); }

  template <typename U>
  constexpr T getValueOr(U &&value) const LVC_LVALUE_FUNCTION {
    return hasValue() ? getValue() : std::forward<U>(value);
  }

  /// Apply a function to the value if present; otherwise return None.
  template <class Function>
  auto map(const Function &F) const LVC_LVALUE_FUNCTION
...
template <class T> lvc::hash_code hash_value(const Optional<T> &O) {
