...
#ifndef LVC_ADT_IMMUTABLELIST_H
#define LVC_ADT_IMMUTABLELIST_H
...
public:
  ...
  template <typename ElemT>
  LVC_NODISCARD ImmutableList<T> concat(ElemT &&Head, ImmutableList<T> Tail) {
  ...
  template <typename ElemT>
  LVC_NODISCARD ImmutableList<T> add(ElemT &&Data, ImmutableList<T> L) {
    return concat(std::forward<ElemT>(Data), L);
  }

  template <typename... CtorArgs>
  LVC_NODISCARD ImmutableList<T> emplace(ImmutableList<T> Tail,
                                          CtorArgs &&...Args) {
