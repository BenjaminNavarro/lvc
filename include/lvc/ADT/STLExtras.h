...
#ifndef LVC_ADT_STLEXTRAS_H
#define LVC_ADT_STLEXTRAS_H
...
auto reverse(ContainerTy &&C,
             std::enable_if_t<!has_rbegin<ContainerTy>::value> * = nullptr) {
  return make_range(lvc::make_reverse_iterator(std::end(C)),
                    lvc::make_reverse_iterator(std::begin(C)));
}
...
auto deref_or_none(const Iter &I, const Iter &End) -> lvc::Optional<
...
template <typename Iter> struct ZipLongestItemType {
  using type =
      lvc::Optional<typename std::remove_const<typename std::remove_reference<
...
/// until all iterators reach the end. The lvc::Optional only contains a value
...
class concat_iterator
  ...
  template <size_t... Ns> void increment(std::index_sequence<Ns...>) {
    ...
    lvc_unreachable("Attempted to increment an end concat iterator!");
  }
  ...
  template <size_t... Ns> ValueT &get(std::index_sequence<Ns...>) const {
    ...
    lvc_unreachable("Attempted to get a pointer from an end concat iterator!");
  }
...
class indexed_accessor_iterator
    : public lvc::iterator_facade_base<DerivedT,
...
inline void presortShuffle(IteratorTy Start, IteratorTy End) {
  ...
  lvc::shuffle(Start, End, Generator);
}
...
template <typename Container> inline void sort(Container &&C) {
  lvc::sort(adl_begin(C), adl_end(C));
}
...
inline void sort(Container &&C, Compare Comp) {
  lvc::sort(adl_begin(C), adl_end(C), Comp);
}
