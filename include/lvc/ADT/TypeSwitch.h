...
#ifndef LVC_ADT_TYPESWITCH_H
#define LVC_ADT_TYPESWITCH_H
...
protected:
  ...
  /// Attempt to dyn_cast the given `value` to `CastT`. This overload is
  /// selected if lvc::dyn_cast should be used.
  template <typename CastT, typename ValueT>
  static auto castValue(
      ValueT value,
      typename std::enable_if_t<
...
public:
  ...
  LVC_NODISCARD ResultT Default(CallableT &&defaultFn) {
    if (result)
