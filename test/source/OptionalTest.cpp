...
private:
  lvc::StringMap<std::unique_ptr<int>> Map;
};
...
#endif // LVC_HAS_RVALUE_REFERENCE_THIS
...
struct ComparableAndStreamable {
  friend bool operator==(ComparableAndStreamable,
                         ComparableAndStreamable) LVC_ATTRIBUTE_USED {
    return true;
  }
...
struct Comparable {
  friend bool operator==(Comparable, Comparable) LVC_ATTRIBUTE_USED {
    return true;
  }
...
TEST(OptionalTest, UseInUnitTests) {
  ...
  // EXPECT macros.
  EXPECT_NONFATAL_FAILURE(EXPECT_EQ(lvc::None, ComparableAndStreamable::get()),
                          "Expected: lvc::None\n"
                          "      Which is: None\n"
  ...
  EXPECT_NONFATAL_FAILURE(EXPECT_EQ(lvc::None, Comparable::get()), "object");
}
