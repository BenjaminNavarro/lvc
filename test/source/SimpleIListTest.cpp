...
#include "lvc/ADT/simple_ilist.h"
...
TEST(SimpleIListTest, merge) {
  for (bool IsL1LHS : {false, true}) {
    ...
    EXPECT_TRUE(lvc::is_sorted(L1));
    EXPECT_TRUE(lvc::is_sorted(L2));
...
TEST(SimpleIListTest, mergeIsStable) {
  ...
  auto setup = [&]() {
    ...
    EXPECT_EQ(2u, L2.size());
    EXPECT_TRUE(lvc::is_sorted(L1, makeFalse));
    EXPECT_TRUE(lvc::is_sorted(L2, makeFalse));
  };
...
TEST(SimpleIListTest, sort) {
  ...
  EXPECT_FALSE(lvc::is_sorted(L));

  // Sort.
  L.sort();
  EXPECT_TRUE(lvc::is_sorted(L));
  auto I = L.begin();
  for (Node &N : Ns)
    EXPECT_EQ(&N, &*I++);
  EXPECT_EQ(L.end(), I);
...
TEST(SimpleIListTest, sortIsStable) {
  ...
  // Check setup.
  EXPECT_EQ(10u, L.size());
  EXPECT_FALSE(lvc::is_sorted(L, compare));

  // Sort.
  L.sort(compare);
  EXPECT_TRUE(lvc::is_sorted(L, compare));
  auto I = L.begin();
  for (int O : {3, 4, 1, 2, 0})
    EXPECT_EQ(&Ns[O], &*I++);
  for (int O : {7, 8, 6, 9, 5})
