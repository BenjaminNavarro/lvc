...
TEST(AnyTest, GoodAnyCast) {
  ...
  EXPECT_EQ(7, lvc::any_cast<int>(B));
  EXPECT_EQ(7, lvc::any_cast<int>(B));

  EXPECT_STREQ("hello", lvc::any_cast<const char *>(D));
  EXPECT_STREQ("hello", lvc::any_cast<const char *>(D));

  EXPECT_EQ('x', lvc::any_cast<char>(E));
  EXPECT_EQ('x', lvc::any_cast<char>(E));

  lvc::Any F(B);
  EXPECT_EQ(7, lvc::any_cast<int>(F));
  EXPECT_EQ(7, lvc::any_cast<int>(F));

  lvc::Any G(std::move(C));
  EXPECT_EQ(8, lvc::any_cast<int>(G));
  EXPECT_EQ(8, lvc::any_cast<int>(G));

  A = F;
  EXPECT_EQ(7, lvc::any_cast<int>(A));
  EXPECT_EQ(7, lvc::any_cast<int>(A));

  E = std::move(G);
  EXPECT_EQ(8, lvc::any_cast<int>(E));
  EXPECT_EQ(8, lvc::any_cast<int>(E));

  // Make sure we can any_cast from an rvalue and that it's properly destroyed
  // in the process.
  EXPECT_EQ(8, lvc::any_cast<int>(std::move(E)));
  EXPECT_TRUE(E.hasValue());

  // Make sure moving from pointers gives back pointers, and that we can modify
  // the underlying value through those pointers.
  EXPECT_EQ(7, *lvc::any_cast<int>(&A));
  int *N = lvc::any_cast<int>(&A);
  *N = 42;
  EXPECT_EQ(42, lvc::any_cast<int>(A));
  ...
  lvc::any_cast<int &>(A) = 43;
  EXPECT_EQ(43, lvc::any_cast<int>(A));
}
...
TEST(AnyTest, BadAnyCast) {
  ...
  EXPECT_DEATH(lvc::any_cast<int>(A), "");

  EXPECT_DEATH(lvc::any_cast<float>(B), "");
  EXPECT_DEATH(lvc::any_cast<int *>(B), "");

  EXPECT_DEATH(lvc::any_cast<std::string>(C), "");

  EXPECT_DEATH(lvc::any_cast<unsigned char>(D), "");
#endif
}
