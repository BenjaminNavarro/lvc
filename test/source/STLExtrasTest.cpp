...
TEST(STLExtrasTest, EnumerateRValueRef) {
  ...
  auto Enumerator = lvc::enumerate(std::vector<int>{1, 2, 3});

  for (auto X : lvc::enumerate(std::vector<int>{1, 2, 3})) {
...
TEST(STLExtrasTest, EnumerateModifyRValue) {
  ...
  for (auto X : lvc::enumerate(std::vector<char>{'1', '2', '3'})) {
    ++X.value();
    Results.emplace_back(X.index(), X.value());
  }
...
TEST(STLExtrasTest, ApplyTupleVariadic) {
  auto Items = std::make_tuple(1, lvc::StringRef("Test"), 'X');
...
TEST(STLExtrasTest, for_each) {
  ...
  lvc::for_each(v, [&count](int) { ++count; });
  EXPECT_EQ(5, count);
}
...
TEST(STLExtrasTest, ADLTest) {
  ...
  int count = 0;
  lvc::for_each(s, [&count](int) { ++count; });
  EXPECT_EQ(5, count);
}

TEST(STLExtrasTest, EmptyTest) {
  std::vector<void*> V;
  EXPECT_TRUE(lvc::empty(V));
  V.push_back(nullptr);
  EXPECT_FALSE(lvc::empty(V));

  std::initializer_list<int> E = {};
  std::initializer_list<int> NotE = {7, 13, 42};
  EXPECT_TRUE(lvc::empty(E));
  EXPECT_FALSE(lvc::empty(NotE));

  auto R0 = make_range(V.begin(), V.begin());
  EXPECT_TRUE(lvc::empty(R0));
  auto R1 = make_range(V.begin(), V.end());
  EXPECT_FALSE(lvc::empty(R1));
}
...
TEST(STLExtrasTest, to_address) {
  ...
  EXPECT_EQ(V2.get(), lvc::to_address(V2));

  V2.reset(V1);
  EXPECT_EQ(V1, lvc::to_address(V2));
  V2.release();
  ...
  EXPECT_EQ(V3.get(), lvc::to_address(V3));
  EXPECT_EQ(V4.get(), lvc::to_address(V4));

  V3.reset(V1);
  EXPECT_EQ(V1, lvc::to_address(V3));
}
...
TEST(STLExtras, MoveRange) {
  ...
  V1.resize(4U);
  EXPECT_TRUE(lvc::all_of(V1, HasVal));

  lvc::move(V1, std::back_inserter(V2));

  // Ensure input container is same size, but its contents were moved out.
  EXPECT_EQ(V1.size(), 4U);
  EXPECT_TRUE(lvc::none_of(V1, HasVal));

  // Ensure output container has the contents of the input container.
  EXPECT_EQ(V2.size(), 4U);
  EXPECT_TRUE(lvc::all_of(V2, HasVal));

  lvc::move(std::move(V2), std::back_inserter(V3));

  EXPECT_TRUE(lvc::none_of(V2, HasVal));
  EXPECT_EQ(V3.size(), 4U);
  EXPECT_TRUE(lvc::all_of(V3, HasVal));

  lvc::move(Build(), std::back_inserter(V4));
  EXPECT_EQ(V4.size(), 4U);
  EXPECT_TRUE(lvc::all_of(V4, HasVal));
}
} // namespace
