...
static std::string convertToErrorFromString(StringRef Str) {
  lvc::APFloat F(0.0);
  auto StatusOrErr =
      F.convertFromString(Str, lvc::APFloat::rmNearestTiesToEven);
...
static std::string convertToString(double d, unsigned Prec, unsigned Pad,
                                   bool Tr = true) {
  lvc::SmallVector<char, 100> Buffer;
  lvc::APFloat F(d);
...
TEST(APFloatTest, fromStringSpecials) {
        ...
        for (char SignChar : Signs) {
          ...
          for (size_t K = 0; K < NumTestStrings; ++K) {
            ...
            bool HasError = !F.convertFromString(
                TestStr, lvc::APFloat::rmNearestTiesToEven);
  ...
  for (StringRef InfStr : InfStrings) {
    ...
    APFloat F(Sem);
    bool HasError =
        !F.convertFromString(InfStr, lvc::APFloat::rmNearestTiesToEven);
    EXPECT_FALSE(HasError);
...
TEST(DoubleAPFloatTest, isInteger) {
  APFloat F1(-0.0);
  APFloat F2(-0.0);
  lvc::detail::DoubleAPFloat T(APFloat::PPCDoubleDouble(), std::move(F1),
                                std::move(F2));
  EXPECT_TRUE(T.isInteger());
  APFloat F3(3.14159);
  APFloat F4(-0.0);
  lvc::detail::DoubleAPFloat T2(APFloat::PPCDoubleDouble(), std::move(F3),
                                std::move(F4));
  EXPECT_FALSE(T2.isInteger());
  APFloat F5(-0.0);
  APFloat F6(3.14159);
  lvc::detail::DoubleAPFloat T3(APFloat::PPCDoubleDouble(), std::move(F5),
                                std::move(F6));
