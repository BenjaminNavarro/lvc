...
TEST(StringSwitchTest, StartsWith) {
  auto Translate = [](StringRef S) {
    return lvc::StringSwitch<std::function<int(int, int)>>(S)
...
TEST(StringSwitchTest, EndsWith) {
  ...
  auto Translate = [](StringRef S) {
    return lvc::StringSwitch<Suffix>(S)
...
TEST(StringSwitchTest, Cases) {
  ...
  auto Translate = [](StringRef S) {
    return lvc::StringSwitch<OSType>(S)
  ...
  EXPECT_EQ(OSType::Windows, Translate(lvc::StringRef("wind\0ws", 7)));
