...
void Z3ErrorHandler(Z3_context Context, Z3_error_code Error) {
  lvc::report_fatal_error("Z3 error: " +
                           lvc::Twine(Z3_get_error_msg(Context, Error)));
}
...
public:
  ...
  void Profile(lvc::FoldingSetNodeID &ID) const override {
    ID.AddInteger(
...
public:
  ...
  LVC_DUMP_METHOD void dump() const { print(lvc::errs()); }
}; // end class Z3Model

/// Get the corresponding IEEE floating-point type for a given bitwidth.
static const lvc::fltSemantics &getFloatSemantics(unsigned BitWidth) {
  switch (BitWidth) {
  default:
    lvc_unreachable("Unsupported floating-point semantics!");
    break;
  case 16:
    return lvc::APFloat::IEEEhalf();
  case 32:
    return lvc::APFloat::IEEEsingle();
  case 64:
    return lvc::APFloat::IEEEdouble();
  case 128:
    return lvc::APFloat::IEEEquad();
  }
}

// Determine whether two float semantics are equivalent
static bool areEquivalent(const lvc::fltSemantics &LHS,
                          const lvc::fltSemantics &RHS) {
  return (lvc::APFloat::semanticsPrecision(LHS) ==
          lvc::APFloat::semanticsPrecision(RHS)) &&
         (lvc::APFloat::semanticsMinExponent(LHS) ==
          lvc::APFloat::semanticsMinExponent(RHS)) &&
         (lvc::APFloat::semanticsMaxExponent(LHS) ==
          lvc::APFloat::semanticsMaxExponent(RHS)) &&
         (lvc::APFloat::semanticsSizeInBits(LHS) ==
          lvc::APFloat::semanticsSizeInBits(RHS));
}
...
public:
  ...
  SMTExprRef mkBitvector(const lvc::APSInt Int, unsigned BitWidth) override {
    const Z3_sort Z3Sort = toZ3Sort(*getBitvectorSort(BitWidth)).Sort;

    // Slow path, when 64 bits are not enough.
    if (LVC_UNLIKELY(Int.getBitWidth() > 64u)) {
  ...
  SMTExprRef mkFloat(const lvc::APFloat Float) override {
    SMTSortRef Sort =
        getFloatSort(lvc::APFloat::semanticsSizeInBits(Float.getSemantics()));

    lvc::APSInt Int = lvc::APSInt(Float.bitcastToAPInt(), false);
  ...
  lvc::APSInt getBitvector(const SMTExprRef &Exp, unsigned BitWidth,
  ...
  bool toAPFloat(const SMTSortRef &Sort, const SMTExprRef &AST,
                 lvc::APFloat &Float, bool useSemantics) {
    assert(Sort->isFloatSort() && "Unsupported sort to floating-point!");

    lvc::APSInt Int(Sort->getFloatSortSize(), true);
    const lvc::fltSemantics &Semantics =
    ...
    Float = lvc::APFloat(Semantics, Int);
    return true;
  }

  bool toAPSInt(const SMTSortRef &Sort, const SMTExprRef &AST,
                lvc::APSInt &Int, bool useSemantics) {
    ...
    if (Sort->isBooleanSort()) {
      ...
      Int = lvc::APSInt(lvc::APInt(Int.getBitWidth(), getBoolean(AST)),
                         Int.isUnsigned());
      return true;
    }

    lvc_unreachable("Unsupported sort to integer!");
  }

  bool getInterpretation(const SMTExprRef &Exp, lvc::APSInt &Int) override {
  ...
  bool getInterpretation(const SMTExprRef &Exp, lvc::APFloat &Float) override {
    Z3Model Model(Context, Z3_solver_get_model(Context.Context, Solver));
    Z3_func_decl Func = Z3_get_app_decl(
        Context.Context, Z3_to_app(Context.Context, toZ3Expr(*Exp).AST));
    if (Z3_model_has_interp(Context.Context, Model.Model, Func) != Z3_L_TRUE)
...
lvc::SMTSolverRef lvc::CreateZ3Solver() {
#if LVC_WITH_Z3
  return std::make_unique<Z3Solver>();
#else
  lvc::report_fatal_error("LLVM was not compiled with Z3 support, rebuild "
                           "with -DLVC_ENABLE_Z3_SOLVER=ON",
                           false);
  return nullptr;
#endif
}

LVC_DUMP_METHOD void SMTSort::dump() const { print(lvc::errs()); }
LVC_DUMP_METHOD void SMTExpr::dump() const { print(lvc::errs()); }
LVC_DUMP_METHOD void SMTSolver::dump() const { print(lvc::errs()); }
