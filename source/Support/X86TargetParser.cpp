...
#include "lvc/Support/X86TargetParser.h"
#include "lvc/ADT/Triple.h"

using namespace lvc;
using namespace lvc::X86;
...
public:
  ...
  bool any() const {
    return lvc::any_of(Bits, [](uint64_t V) { return V != 0; });
  }
...
#include "lvc/Support/X86TargetParser.def"
...
X86::CPUKind lvc::X86::parseArchX86(StringRef CPU, bool Only64Bit) {
...
X86::CPUKind lvc::X86::parseTuneCPU(StringRef CPU, bool Only64Bit) {
  if (lvc::is_contained(NoTuneList, CPU))
    return CK_None;
  return parseArchX86(CPU, Only64Bit);
}

void lvc::X86::fillValidCPUArchList(SmallVectorImpl<StringRef> &Values,
                                     bool Only64Bit) {
  for (const auto &P : Processors)
...
void lvc::X86::fillValidTuneCPUList(SmallVectorImpl<StringRef> &Values,
                                     bool Only64Bit) {
  for (const ProcInfo &P : Processors)
    if (!P.Name.empty() && (P.Features[FEATURE_64BIT] || !Only64Bit) &&
        !lvc::is_contained(NoTuneList, P.Name))
      Values.emplace_back(P.Name);
}

ProcessorFeatures lvc::X86::getKeyFeature(X86::CPUKind Kind) {
  ...
  lvc_unreachable("Unable to find CPU kind!");
}
...
void lvc::X86::getFeaturesForCPU(StringRef CPU,
                                  SmallVectorImpl<StringRef> &EnabledFeatures) {
  auto I = lvc::find_if(Processors,
...
void lvc::X86::updateImpliedFeatures(
