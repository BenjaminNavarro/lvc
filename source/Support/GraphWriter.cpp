...
#include "lvc/Support/GraphWriter.h"
...
std::string lvc::DOT::EscapeString(const std::string &Label) {
...
StringRef lvc::DOT::getColorString(unsigned ColorNumber) {
...
std::string lvc::createGraphFilename(const Twine &Name, int &FD) {
  FD = -1;
...
static const char *getProgramName(GraphProgram::Name program) {
  ...
  lvc_unreachable("bad kind");
}

bool lvc::DisplayGraph(StringRef FilenameRef, bool wait,
  ...
  if (Viewer &&
    ...
    case VK_CmdStart:
      ...
      break;
    case VK_None:
      lvc_unreachable("Invalid viewer");
    }
