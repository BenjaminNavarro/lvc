...
#include "lvc/ADT/APFloat.h"
#include "lvc/ADT/APSInt.h"
...
#define APFLOAT_DISPATCH_ON_SEMANTICS(METHOD_CALL)                             \
  do {                                                                         \
    ...
    lvc_unreachable("Unexpected semantics");                                  \
  } while (false)
...
namespace lvc {
  ...
  const lvc::fltSemantics &APFloatBase::EnumToSemantics(Semantics S) {
    switch (S) {
    case S_IEEEhalf:
      return IEEEhalf();
    case S_BFloat:
    ...
    lvc_unreachable("Unrecognised floating semantics");
  }

  APFloatBase::Semantics
  APFloatBase::SemanticsToEnum(const lvc::fltSemantics &Sem) {
    if (&Sem == &lvc::APFloat::IEEEhalf())
      return S_IEEEhalf;
    else if (&Sem == &lvc::APFloat::BFloat())
      return S_BFloat;
    else if (&Sem == &lvc::APFloat::IEEEsingle())
      return S_IEEEsingle;
    else if (&Sem == &lvc::APFloat::IEEEdouble())
      return S_IEEEdouble;
    else if (&Sem == &lvc::APFloat::x87DoubleExtended())
      return S_x87DoubleExtended;
    else if (&Sem == &lvc::APFloat::IEEEquad())
      return S_IEEEquad;
    else if (&Sem == &lvc::APFloat::PPCDoubleDouble())
      return S_PPCDoubleDouble;
    else
      lvc_unreachable("Unknown floating semantics");
  }
...
bool IEEEFloat::roundAwayFromZero(roundingMode rounding_mode,
  ...
  lvc_unreachable("Invalid rounding mode found");
}
...
APInt IEEEFloat::convertF80LongDoubleAPFloatToAPInt() const {
  assert(semantics == (const lvc::fltSemantics*)&semX87DoubleExtended);
  assert(partCount()==2);
...
APInt IEEEFloat::convertPPCDoubleDoubleAPFloatToAPInt() const {
  assert(semantics == (const lvc::fltSemantics *)&semPPCDoubleDoubleLegacy);
  assert(partCount()==2);
...
APInt IEEEFloat::convertQuadrupleAPFloatToAPInt() const {
  assert(semantics == (const lvc::fltSemantics*)&semIEEEquad);
  assert(partCount()==2);
...
APInt IEEEFloat::convertDoubleAPFloatToAPInt() const {
  assert(semantics == (const lvc::fltSemantics*)&semIEEEdouble);
  assert(partCount()==1);
...
APInt IEEEFloat::convertFloatAPFloatToAPInt() const {
  assert(semantics == (const lvc::fltSemantics*)&semIEEEsingle);
  assert(partCount()==1);
...
APInt IEEEFloat::convertBFloatAPFloatToAPInt() const {
  assert(semantics == (const lvc::fltSemantics *)&semBFloat);
  assert(partCount() == 1);
...
APInt IEEEFloat::convertHalfAPFloatToAPInt() const {
  assert(semantics == (const lvc::fltSemantics*)&semIEEEhalf);
  assert(partCount()==1);
...
APInt IEEEFloat::bitcastToAPInt() const {
  if (semantics == (const lvc::fltSemantics*)&semIEEEhalf)
    return convertHalfAPFloatToAPInt();

  if (semantics == (const lvc::fltSemantics *)&semBFloat)
    return convertBFloatAPFloatToAPInt();

  if (semantics == (const lvc::fltSemantics*)&semIEEEsingle)
    return convertFloatAPFloatToAPInt();

  if (semantics == (const lvc::fltSemantics*)&semIEEEdouble)
    return convertDoubleAPFloatToAPInt();

  if (semantics == (const lvc::fltSemantics*)&semIEEEquad)
    return convertQuadrupleAPFloatToAPInt();

  if (semantics == (const lvc::fltSemantics *)&semPPCDoubleDoubleLegacy)
...
LVC_DUMP_METHOD void APFloat::dump() const { print(dbgs()); }
#endif
