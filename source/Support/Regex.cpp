...
Regex::Regex(StringRef regex, RegexFlags Flags) {
  ...
  if (!(Flags & BasicRegex))
    flags |= REG_EXTENDED;
  error = LVC_regcomp(preg, regex.data(), flags|REG_PEND);
}
...
void RegexErrorToString(int error, struct LVC_regex *preg,
                        std::string &Error) {
  size_t len = LVC_regerror(error, preg, nullptr, 0);

  Error.resize(len - 1);
  LVC_regerror(error, preg, &Error[0], len);
}
...
bool Regex::match(StringRef String, SmallVectorImpl<StringRef> *Matches,
  ...
  SmallVector<LVC_regmatch_t, 8> pm;
  pm.resize(nmatch > 0 ? nmatch : 1);
  pm[0].rm_so = 0;
  pm[0].rm_eo = String.size();

  int rc = LVC_regexec(preg, String.data(), nmatch, pm.data(), REG_STARTEND);
