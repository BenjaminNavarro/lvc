...
#include "lvc/Support/StringSaver.h"
...
OptionCategory lvc::cl::GeneralCategory("General options");
...
LVC_REQUIRE_CONSTANT_INITIALIZATION
ManagedStatic<SubCommand> lvc::cl::TopLevelSubCommand;

// A special subcommand that can be used to put an option into all subcommands.
ManagedStatic<SubCommand> lvc::cl::AllSubCommands;
...
bool lvc::cl::ProvidePositionalOption(Option *Handler, StringRef Arg, int i) {
  int Dummy = i;
  ...
  for (size_t I = 0, E = Src.size(); I < E; ++I) {
    ...
    case INIT: {
      ...
      if (I >= E || isWhitespaceOrNull(Src[I])) {
        ...
        lvc_unreachable("unexpected special character");
      }
      break;
    }
...
static lvc::Error ExpandResponseFile(
    ...
    lvc::vfs::FileSystem &FS) {
  assert(sys::path::is_absolute(FName));
  lvc::ErrorOr<std::unique_ptr<MemoryBuffer>> MemBufOrErr =
      FS.getBufferForFile(FName);
  if (!MemBufOrErr)
    return lvc::errorCodeToError(MemBufOrErr.getError());
  ...
  if (hasUTF16ByteOrderMark(BufRef)) {
    if (!convertUTF16ToUTF8String(BufRef, UTF8Buf))
      return lvc::createStringError(std::errc::illegal_byte_sequence,
  ...
  lvc::StringRef BasePath = lvc::sys::path::parent_path(FName);
  ...
  for (auto &Arg : NewArgv) {
    ...
    if (!lvc::sys::path::is_relative(FileName))
      continue;

    SmallString<128> ResponseFile;
    ResponseFile.push_back('@');
    ResponseFile.append(BasePath);
    lvc::sys::path::append(ResponseFile, FileName);
...
bool cl::ExpandResponseFiles(StringSaver &Saver, TokenizerCallback Tokenizer,
                             ...
                             bool RelativeNames, lvc::vfs::FileSystem &FS,
                             lvc::Optional<lvc::StringRef> CurrentDir) {
  ...
  for (unsigned I = 0; I != Argv.size();) {
    ...
    if (lvc::sys::path::is_relative(FName)) {
      if (!CurrentDir)
        lvc::sys::fs::current_path(CurrDir);
      else
        CurrDir = *CurrentDir;
      lvc::sys::path::append(CurrDir, FName);
      FName = CurrDir.c_str();
    }
    auto IsEquivalent = [FName, &FS](const ResponseFileRecord &RFile) {
      lvc::ErrorOr<lvc::vfs::Status> LHS = FS.status(FName);
      if (!LHS) {
        // TODO: The error should be propagated up the stack.
        lvc::consumeError(lvc::errorCodeToError(LHS.getError()));
        return false;
      }
      lvc::ErrorOr<lvc::vfs::Status> RHS = FS.status(RFile.File);
      if (!RHS) {
        // TODO: The error should be propagated up the stack.
        lvc::consumeError(lvc::errorCodeToError(RHS.getError()));
        return false;
      }
...
bool cl::expandResponseFiles(int Argc, const char *const *Argv,
  ...
  if (EnvVar)
    if (lvc::Optional<std::string> EnvValue = sys::Process::GetEnv(EnvVar))
...
bool cl::readConfigFile(StringRef CfgFile, StringSaver &Saver,
  ...
  if (sys::path::is_relative(CfgFile)) {
    lvc::sys::fs::current_path(AbsPath);
    lvc::sys::path::append(AbsPath, CfgFile);
    CfgFile = AbsPath.str();
  }
  if (lvc::Error Err =
          ExpandResponseFile(CfgFile, Saver, cl::tokenizeConfigFile, Argv,
                             /*MarkEOLs*/ false, /*RelativeNames*/ true,
                             *lvc::vfs::getRealFileSystem())) {
    // TODO: The error should be propagated up the stack.
    lvc::consumeError(std::move(Err));
    return false;
  }
...
bool cl::ParseCommandLineOptions(int argc, const char *const *argv,
  ...
  if (EnvVar) {
    if (lvc::Optional<std::string> EnvValue =
    ...
    for (size_t i = 0, e = PositionalOpts.size(); i != e; ++i) {
      ...
      while (NumVals - ValNo > NumPositionalRequired && !Done) {
        ...
        case cl::OneOrMore:  // One or more will take all they can get...
          ...
          break;
        default:
          lvc_unreachable("Internal error, unexpected NumOccurrences flag in "
...
#  define LVC_IS_DEBUG_BUILD 0
# else
#  define LVC_IS_DEBUG_BUILD 1
# endif
...
public:
  void print() {
...
#ifdef LVC_VERSION_INFO
    OS << " " << LVC_VERSION_INFO;
#endif
    OS << "\n  ";
#if LVC_IS_DEBUG_BUILD
    OS << "DEBUG build";
#else
    OS << "Optimized build";
#endif
    ...
    OS << " with assertions";
#endif
#if LVC_VERSION_PRINTER_SHOW_HOST_TARGET_INFO
...
void LLVMParseCommandLineOptions(int argc, const char *const *argv,
                                 const char *Overview) {
  lvc::cl::ParseCommandLineOptions(argc, argv, StringRef(Overview),
                                    &lvc::nulls());
}
