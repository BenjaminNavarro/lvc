...
#include "lvc/Support/Mutex.h"
#include "lvc/Support/Timer.h"
#include "lvc/Support/YAMLTraits.h"
...
/// the first statistic is bumped) and destroyed only when LVC_shutdown is
...
class StatisticInfo {
  ...
  friend void lvc::PrintStatistics();
  friend void lvc::PrintStatistics(raw_ostream &OS);
  friend void lvc::PrintStatisticsJSON(raw_ostream &OS);
...
void TrackingStatistic::RegisterStatistic() {
  ...
  // printed.
  // LVC_shutdown calls destructors while holding the ManagedStatic mutex.
...
void lvc::EnableStatistics(bool DoPrintOnExit) {
  Enabled = true;
  PrintOnExit = DoPrintOnExit;
}

bool lvc::AreStatisticsEnabled() {
...
void lvc::PrintStatistics(raw_ostream &OS) {
...
void lvc::PrintStatisticsJSON(raw_ostream &OS) {
...
void lvc::PrintStatistics() {
  ...
  if (EnableStats) {
    ...
    (*OutStream) << "Statistics are disabled.  "
                 << "Build with asserts or with -DLVC_FORCE_ENABLE_STATS\n";
  }
#endif
}

const std::vector<std::pair<StringRef, unsigned>> lvc::GetStatistics() {
  sys::SmartScopedLock<true> Reader(*StatLock);
