...
#include "lvc/Support/FileUtilities.h"
...
/// file system error.
///
int lvc::DiffFilesWithTolerance(StringRef NameA,
                                 StringRef NameB,
...
void lvc::AtomicFileWriteError::log(raw_ostream &OS) const {
  ...
  lvc_unreachable("unknown atomic_write_error value in "
                   "failed_to_rename_temp_file::log()");
}

lvc::Error lvc::writeFileAtomically(StringRef TempPathModel,
                                      StringRef FinalPath, StringRef Buffer) {
  return writeFileAtomically(TempPathModel, FinalPath,
                             [&Buffer](lvc::raw_ostream &OS) {
                               OS.write(Buffer.data(), Buffer.size());
                               return lvc::Error::success();
                             });
}

lvc::Error lvc::writeFileAtomically(
    StringRef TempPathModel, StringRef FinalPath,
    std::function<lvc::Error(lvc::raw_ostream &)> Writer) {
  SmallString<128> GeneratedUniqPath;
  int TempFD;
  if (sys::fs::createUniqueFile(TempPathModel.str(), TempFD,
                                GeneratedUniqPath)) {
    return lvc::make_error<AtomicFileWriteError>(
        atomic_write_error::failed_to_create_uniq_file);
  }
  lvc::FileRemover RemoveTmpFileOnFail(GeneratedUniqPath);

  raw_fd_ostream OS(TempFD, /*shouldClose=*/true);
  if (lvc::Error Err = Writer(OS)) {
    return Err;
  }
...
char lvc::AtomicFileWriteError::ID;
