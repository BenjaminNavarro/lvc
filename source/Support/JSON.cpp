...
#include "lvc/Support/JSON.h"
#include "lvc/ADT/STLExtras.h"
#include "lvc/Support/ConvertUTF.h"
#include "lvc/Support/Error.h"
#include "lvc/Support/Format.h"
...
lvc::Optional<std::nullptr_t> Object::getNull(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsNull();
  return lvc::None;
}
lvc::Optional<bool> Object::getBoolean(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsBoolean();
  return lvc::None;
}
lvc::Optional<double> Object::getNumber(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsNumber();
  return lvc::None;
}
lvc::Optional<int64_t> Object::getInteger(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsInteger();
  return lvc::None;
}
lvc::Optional<lvc::StringRef> Object::getString(StringRef K) const {
  if (auto *V = get(K))
...
bool operator==(const Value &L, const Value &R) {
  ...
  lvc_unreachable("Unknown value kind");
}

void Path::report(lvc::StringLiteral Msg) {
...
Error Path::Root::getError() const {
  ...
  if (ErrorPath.empty()) {
    ...
    for (const Path::Segment &S : lvc::reverse(ErrorPath)) {
      if (S.isField())
        OS << '.' << S.field();
      else
        OS << '[' << S.index() << ']';
  ...
  return createStringError(lvc::inconvertibleErrorCode(), OS.str());
}
...
void Path::Root::printErrorContext(const Value &R, raw_ostream &OS) const {
  ...
  auto PrintValue = [&](const Value &V, ArrayRef<Segment> Path, auto &Recurse) {
    ...
    if (S.isField()) {
      ...
      lvc::StringRef FieldName = S.field();
...
bool Parser::parseString(std::string &Out) {
  ...
  for (char C = next(); C != '"'; C = next()) {
    if (LVC_UNLIKELY(P == End))
      return parseError("Unterminated string");
    if (LVC_UNLIKELY((C & 0x1f) == C))
...
static void encodeUtf8(uint32_t Rune, std::string &Out) {
  if (Rune < 0x80) {
    ...
    lvc_unreachable("Invalid codepoint");
  }
}
...
bool Parser::parseUnicode(std::string &Out) {
  ...
  while (true) {
    ...
    if (LVC_LIKELY(First < 0xD800 || First >= 0xE000)) {
      encodeUtf8(First, Out);
      return true;
    }

    // Case 2: it's an (unpaired) trailing surrogate.
    if (LVC_UNLIKELY(First >= 0xDC00)) {
      Invalid();
      return true;
    }
    ...
    if (LVC_UNLIKELY(P + 2 > End || *P != '\\' || *(P + 1) != 'u')) {
    ...
    if (LVC_UNLIKELY(Second < 0xDC00 || Second >= 0xE000)) {
      Invalid();      // Leading surrogate was unpaired.
...
bool isUTF8(lvc::StringRef S, size_t *ErrOffset) {
  // Fast-path for ASCII, which is valid UTF-8.
  if (LVC_LIKELY(isASCII(S)))
    return true;

  const UTF8 *Data = reinterpret_cast<const UTF8 *>(S.data()), *Rest = Data;
  if (LVC_LIKELY(isLegalUTF8String(&Rest, Data + S.size())))
    return true;
...
static void quote(lvc::raw_ostream &OS, lvc::StringRef S) {
  OS << '\"';
  for (unsigned char C : S) {
    ...
    default:
      OS << 'u';
      lvc::write_hex(OS, C, lvc::HexPrintStyle::Lower, 4);
      break;
    }
  }
  OS << '\"';
...
void lvc::json::OStream::value(const Value &V) {
  switch (V.kind()) {
  case Value::Null:
    valueBegin();
    OS << "null";
...
void lvc::json::OStream::valueBegin() {
...
void OStream::comment(lvc::StringRef Comment) {
...
void lvc::json::OStream::arrayBegin() {
  valueBegin();
  Stack.emplace_back();
...
void lvc::json::OStream::arrayEnd() {
...
void lvc::json::OStream::objectBegin() {
  valueBegin();
  Stack.emplace_back();
...
void lvc::json::OStream::objectEnd() {
...
void lvc::json::OStream::attributeBegin(lvc::StringRef Key) {
  assert(Stack.back().Ctx == Object);
  if (Stack.back().HasValue)
    OS << ',';
  newline();
...
void lvc::json::OStream::attributeEnd() {
...
raw_ostream &lvc::json::OStream::rawValueBegin() {
  valueBegin();
  Stack.emplace_back();
...
void lvc::json::OStream::rawValueEnd() {
...
void lvc::format_provider<lvc::json::Value>::format(
    const lvc::json::Value &E, raw_ostream &OS, StringRef Options) {
  unsigned IndentAmount = 0;
  if (!Options.empty() && Options.getAsInteger(/*Radix=*/10, IndentAmount))
    lvc_unreachable("json::Value format options should be an integer");
