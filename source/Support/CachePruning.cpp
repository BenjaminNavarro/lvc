...
#include "lvc/Support/CachePruning.h"
...
lvc::parseCachePruningPolicy(StringRef PolicyStr) {
...
bool lvc::pruneCache(StringRef Path, CachePruningPolicy Policy) {
  ...
  if (Policy.Expiration == seconds(0) &&
    ...
    LVC_DEBUG(dbgs() << "No pruning settings set, exit early\n");
  ...
  if (auto EC = sys::fs::status(TimestampFile, FileStatus)) {
    ...
    if (Policy.Interval != seconds(0)) {
      ...
      if (TimeStampAge <= *Policy.Interval) {
        LVC_DEBUG(dbgs() << "Timestamp file too recent ("
  ...
  for (sys::fs::directory_iterator File(CachePathNative, EC), FileEnd;
    ...
    if (!StatusOrErr) {
      LVC_DEBUG(dbgs() << "Ignore " << File->path() << " (can't stat)\n");
      continue;
    }
    ...
    if (Policy.Expiration != seconds(0) && FileAge > Policy.Expiration) {
      LVC_DEBUG(dbgs() << "Remove " << File->path() << " ("
  ...
  auto RemoveCacheFile = [&]() {
    ...
    NumFiles--;
    LVC_DEBUG(dbgs() << " - Remove " << FileInfo->Path << " (size "
  ...
  if (Policy.MaxSizePercentageOfAvailableSpace > 0 || Policy.MaxSizeBytes > 0) {
    ...
    LVC_DEBUG(dbgs() << "Occupancy: " << ((100 * TotalSize) / AvailableSpace)
                      << "% target is: "
                      << Policy.MaxSizePercentageOfAvailableSpace << "%, "
