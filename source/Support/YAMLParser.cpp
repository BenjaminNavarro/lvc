...
#include "lvc/ADT/AllocatorList.h"
  ...
  for (StringRef::iterator i = Input.begin(), e = Input.end(); i != e; ++i) {
    if (*i == '\\')
      ...
      if (UnicodeScalarValue.second == 0) {
        ...
        lvc::append_range(EscapedInput, Val);
...
lvc::Optional<bool> yaml::parseBool(StringRef S) {
  switch (S.size()) {
  case 1:
    switch (S.front()) {
    case 'y':
...
LVC_ATTRIBUTE_NOINLINE static bool
  ...
  if (Value[0] == '"') { // Double quoted.
    ...
    if (i != StringRef::npos) {
      ...
      for (; i != StringRef::npos; i = UnquotedValue.find('\'')) {
        ...
        lvc::append_range(Storage, Valid);
        Storage.push_back('\'');
        UnquotedValue = UnquotedValue.substr(i + 2);
      }
      lvc::append_range(Storage, UnquotedValue);
...
parse_property:
  ...
  lvc_unreachable("Control flow shouldn't reach here.");
  return nullptr;
}
