...
#include "lvc/Support/LineIterator.h"
...
void IO::setAllowUnknownKeys(bool Allow) {
  lvc_unreachable("Only supported for Input");
}
...
NodeKind Input::getNodeKind() {
  ...
  lvc_unreachable("Unsupported node kind");
}
...
void Output::endEnumScalar() {
  if (!EnumerationMatchFound)
    lvc_unreachable("bad runtime enum value");
}
...
StringRef ScalarTraits<bool>::input(StringRef Scalar, void *, bool &Val) {
  if (lvc::Optional<bool> Parsed = parseBool(Scalar)) {
    Val = *Parsed;
    return StringRef();
  }
