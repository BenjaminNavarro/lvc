...
#include "lvc/Support/ItaniumManglingCanonicalizer.h"
#include "lvc/ADT/DenseMap.h"
#include "lvc/ADT/FoldingSet.h"
#include "lvc/ADT/StringRef.h"
#include "lvc/Demangle/ItaniumDemangle.h"
#include "lvc/Support/Allocator.h"

using namespace lvc;
using lvc::itanium_demangle::ForwardTemplateReference;
using lvc::itanium_demangle::Node;
using lvc::itanium_demangle::NodeKind;
using lvc::itanium_demangle::StringView;

namespace {
struct FoldingSetNodeIDBuilder {
  lvc::FoldingSetNodeID &ID;
  void operator()(const Node *P) { ID.AddPointer(P); }
  void operator()(StringView Str) {
    ID.AddString(lvc::StringRef(Str.begin(), Str.size()));
  }
  template <typename T>
...
template<typename ...T>
void profileCtor(lvc::FoldingSetNodeID &ID, Node::Kind K, T ...V) {
...
template<> void ProfileNode::operator()(const ForwardTemplateReference *N) {
  lvc_unreachable("should never canonicalize a ForwardTemplateReference");
}

void profileNode(lvc::FoldingSetNodeID &ID, const Node *N) {
  N->visit(ProfileNode{ID});
}

class FoldingNodeAllocator {
  class alignas(alignof(Node *)) NodeHeader : public lvc::FoldingSetNode {
  public:
    ...
    void Profile(lvc::FoldingSetNodeID &ID) { profileNode(ID, getNode()); }
  };

  BumpPtrAllocator RawAlloc;
  lvc::FoldingSet<NodeHeader> Nodes;
...
class CanonicalizerAllocator : public FoldingNodeAllocator {
  ...
  lvc::SmallDenseMap<Node*, Node*, 32> Remappings;
