...
#include "lvc/ADT/iterator_range.h"
#include "lvc/Config/llvm-config.h"
#include "lvc/Support/Casting.h"
#include "lvc/Support/Chrono.h"
#include "lvc/Support/Compiler.h"
#include "lvc/Support/Debug.h"
#include "lvc/Support/Errc.h"
#include "lvc/Support/ErrorHandling.h"
#include "lvc/Support/ErrorOr.h"
#include "lvc/Support/FileSystem.h"
#include "lvc/Support/MemoryBuffer.h"
#include "lvc/Support/Path.h"
#include "lvc/Support/Process.h"
#include "lvc/Support/SMLoc.h"
#include "lvc/Support/SourceMgr.h"
#include "lvc/Support/YAMLParser.h"
...
using namespace lvc;
using namespace lvc::vfs;

using lvc::sys::fs::file_t;
using lvc::sys::fs::file_status;
using lvc::sys::fs::file_type;
using lvc::sys::fs::kInvalidFile;
using lvc::sys::fs::perms;
using lvc::sys::fs::UniqueID;
...
FileSystem::getBufferForFile(const lvc::Twine &Name, int64_t FileSize,
...
std::error_code FileSystem::makeAbsolute(SmallVectorImpl<char> &Path) const {
  if (lvc::sys::path::is_absolute(Path))
    return {};
  ...
  lvc::sys::fs::make_absolute(WorkingDir.get(), Path);
  return {};
}
...
static bool pathHasTraversal(StringRef Path) {
  using namespace lvc::sys;

  for (StringRef Comp : lvc::make_range(path::begin(Path), path::end(Path)))
...
class RealFile : public File {
  ...
  RealFile(file_t RawFD, StringRef NewName, StringRef NewRealPathName)
      : FD(RawFD), S(NewName, {}, {}, {}, {}, {},
                     lvc::sys::fs::file_type::status_error, {}),
...
/// sending absolute paths to lvc::sys::fs:: functions.
/// A more principled approach would be to push this down a level, modelling
/// the working dir as an lvc::sys::fs::WorkingDir or similar.
...
public:
  explicit RealFileSystem(bool LinkCWDToProcess) {
    if (!LinkCWDToProcess) {
      ...
      if (lvc::sys::fs::current_path(PWD))
        return; // Awful, but nothing to do here.
      if (lvc::sys::fs::real_path(PWD, RealPWD))
        WD = {PWD, PWD};
      else
        WD = {PWD, RealPWD};
    }
  ...
  lvc::ErrorOr<std::string> getCurrentWorkingDirectory() const override;
...
lvc::ErrorOr<std::string> RealFileSystem::getCurrentWorkingDirectory() const {
  if (WD)
    return std::string(WD->Specified.str());

  SmallString<128> Dir;
  if (std::error_code EC = lvc::sys::fs::current_path(Dir))
    return EC;
...
std::error_code RealFileSystem::setCurrentWorkingDirectory(const Twine &Path) {
  if (!WD)
    return lvc::sys::fs::set_current_path(Path);

  SmallString<128> Absolute, Resolved, Storage;
  adjustPath(Path, Storage).toVector(Absolute);
  bool IsDir;
  if (auto Err = lvc::sys::fs::is_directory(Absolute, IsDir))
    return Err;
  if (!IsDir)
    return std::make_error_code(std::errc::not_a_directory);
  if (auto Err = lvc::sys::fs::real_path(Absolute, Resolved))
    return Err;
...
std::error_code RealFileSystem::isLocal(const Twine &Path, bool &Result) {
  SmallString<256> Storage;
  return lvc::sys::fs::is_local(adjustPath(Path, Storage), Result);
}
...
RealFileSystem::getRealPath(const Twine &Path,
                            SmallVectorImpl<char> &Output) const {
  SmallString<256> Storage;
  return lvc::sys::fs::real_path(adjustPath(Path, Storage), Output);
}
...
class RealFSDirIter : public lvc::vfs::detail::DirIterImpl {
  lvc::sys::fs::directory_iterator Iter;

public:
  RealFSDirIter(const Twine &Path, std::error_code &EC) : Iter(Path, EC) {
    if (Iter != lvc::sys::fs::directory_iterator())
  ...
  std::error_code increment() override {
    std::error_code EC;
    Iter.increment(EC);
    CurrentEntry = (Iter == lvc::sys::fs::directory_iterator())
                       ? directory_entry()
...
ErrorOr<Status> OverlayFileSystem::status(const Twine &Path) {
  ...
  for (iterator I = overlays_begin(), E = overlays_end(); I != E; ++I) {
    ...
    if (Status || Status.getError() != lvc::errc::no_such_file_or_directory)
      return Status;
  }
  return make_error_code(lvc::errc::no_such_file_or_directory);
}

ErrorOr<std::unique_ptr<File>>
OverlayFileSystem::openFileForRead(const lvc::Twine &Path) {
  // FIXME: handle symlinks that cross file systems
  for (iterator I = overlays_begin(), E = overlays_end(); I != E; ++I) {
    auto Result = (*I)->openFileForRead(Path);
    if (Result || Result.getError() != lvc::errc::no_such_file_or_directory)
      return Result;
  }
  return make_error_code(lvc::errc::no_such_file_or_directory);
}

lvc::ErrorOr<std::string>
...
lvc::vfs::detail::DirIterImpl::~DirIterImpl() = default;

namespace {

/// Combines and deduplicates directory entries across multiple file systems.
class CombiningDirIterImpl : public lvc::vfs::detail::DirIterImpl {
  using FileSystemPtr = lvc::IntrusiveRefCntPtr<lvc::vfs::FileSystem>;
  ...
  std::error_code incrementImpl(bool IsFirstTime) {
    while (true) {
      ...
      StringRef Name = lvc::sys::path::filename(CurrentEntry.path());
      if (SeenNames.insert(Name).second)
        return EC; // name not seen before
    }
    lvc_unreachable("returned above");
  }
...
public:
  InMemoryNode(lvc::StringRef FileName, InMemoryNodeKind Kind)
      : Kind(Kind), FileName(std::string(lvc::sys::path::filename(FileName))) {
  }
...
class InMemoryFile : public InMemoryNode {
  Status Stat;
  std::unique_ptr<lvc::MemoryBuffer> Buffer;

public:
  InMemoryFile(Status Stat, std::unique_ptr<lvc::MemoryBuffer> Buffer)
  ...
  lvc::MemoryBuffer *getBuffer() const { return Buffer.get(); }
...
public:
  ...
  lvc::ErrorOr<Status> status() override {
    return Node.getStatus(RequestedName);
  }

  lvc::ErrorOr<std::unique_ptr<lvc::MemoryBuffer>>
  getBuffer(const Twine &Name, int64_t FileSize, bool RequiresNullTerminator,
            bool IsVolatile) override {
    lvc::MemoryBuffer *Buf = Node.getBuffer();
    return lvc::MemoryBuffer::getMemBuffer(
...
class InMemoryDirectory : public InMemoryNode {
  Status Stat;
  lvc::StringMap<std::unique_ptr<InMemoryNode>> Entries;
...
Status getNodeStatus(const InMemoryNode *Node, const Twine &RequestedName) {
  ...
  lvc_unreachable("Unknown node type");
}
} // namespace
} // namespace detail
...
InMemoryFileSystem::InMemoryFileSystem(bool UseNormalizedPaths)
    : Root(new detail::InMemoryDirectory(
          Status("", getNextVirtualUniqueID(), lvc::sys::TimePoint<>(), 0, 0,
                 0, lvc::sys::fs::file_type::directory_file,
                 lvc::sys::fs::perms::all_all))),
...
bool InMemoryFileSystem::addFile(const Twine &P, time_t ModificationTime,
                                 ...
                                 Optional<lvc::sys::fs::file_type> Type,
                                 Optional<lvc::sys::fs::perms> Perms,
  ...
  if (useNormalizedPaths())
    lvc::sys::path::remove_dots(Path, /*remove_dot_dot=*/true);

  if (Path.empty())
    return false;

  detail::InMemoryDirectory *Dir = Root.get();
  auto I = lvc::sys::path::begin(Path), E = sys::path::end(Path);
  ...
  while (true) {
    ...
    if (!Node) {
      if (I == E) {
        ...
        if (HardLinkTarget)
          ...
          Status Stat(P.str(), getNextVirtualUniqueID(),
                      lvc::sys::toTimePoint(ModificationTime), ResolvedUser,
      ...
      Status Stat(
          ...
          getNextVirtualUniqueID(), lvc::sys::toTimePoint(ModificationTime),
...
bool InMemoryFileSystem::addFileNoOwn(const Twine &P, time_t ModificationTime,
                                      const lvc::MemoryBufferRef &Buffer,
                                      Optional<uint32_t> User,
                                      Optional<uint32_t> Group,
                                      Optional<lvc::sys::fs::file_type> Type,
                                      Optional<lvc::sys::fs::perms> Perms) {
  return addFile(P, ModificationTime, lvc::MemoryBuffer::getMemBuffer(Buffer),
...
lookupInMemoryNode(const InMemoryFileSystem &FS, detail::InMemoryDirectory *Dir,
  ...
  auto I = lvc::sys::path::begin(Path), E = lvc::sys::path::end(Path);
  while (true) {
...
lvc::ErrorOr<Status> InMemoryFileSystem::status(const Twine &Path) {
...
lvc::ErrorOr<std::unique_ptr<File>>
InMemoryFileSystem::openFileForRead(const Twine &Path) {
  ...
  return make_error_code(lvc::errc::invalid_argument);
}

namespace {

/// Adaptor from InMemoryDir::iterator to directory_iterator.
class InMemoryDirIterator : public lvc::vfs::detail::DirIterImpl {
  ...
  void setCurrentEntry() {
    if (I != E) {
      ...
      lvc::sys::path::append(Path, I->second->getFileName());
...
directory_iterator InMemoryFileSystem::dir_begin(const Twine &Dir,
  ...
  EC = make_error_code(lvc::errc::not_a_directory);
  return directory_iterator(std::make_shared<InMemoryDirIterator>());
}
...
InMemoryFileSystem::getRealPath(const Twine &Path,
  ...
  if (auto EC = makeAbsolute(Output))
    return EC;
  lvc::sys::path::remove_dots(Output, /*remove_dot_dot=*/true);
  return {};
}
...
static lvc::sys::path::Style getExistingStyle(lvc::StringRef Path) {
  // Detect the path style in use by checking the first separator.
  lvc::sys::path::Style style = lvc::sys::path::Style::native;
  const size_t n = Path.find_first_of("/\\");
  if (n != static_cast<size_t>(-1))
    style = (Path[n] == '/') ? lvc::sys::path::Style::posix
                             : lvc::sys::path::Style::windows;
  return style;
}

/// Removes leading "./" as well as path components like ".." and ".".
static lvc::SmallString<256> canonicalize(lvc::StringRef Path) {
  // First detect the path style in use by checking the first separator.
  lvc::sys::path::Style style = getExistingStyle(Path);

  // Now remove the dots.  Explicitly specifying the path style prevents the
  // direction of the slashes from changing.
  lvc::SmallString<256> result =
      lvc::sys::path::remove_leading_dotslash(Path, style);
  lvc::sys::path::remove_dots(result, /*remove_dot_dot=*/true, style);
  return result;
}
...
/// directory entries.
class lvc::vfs::RedirectingFSDirIterImpl
    : public lvc::vfs::detail::DirIterImpl {
  std::string Dir;
  ...
  std::error_code incrementImpl(bool IsFirstTime) {
    ...
    if (Current != End) {
      ...
      lvc::sys::path::append(PathStr, (*Current)->getName());
      sys::fs::file_type Type = sys::fs::file_type::type_unknown;
...
class RedirectingFSDirRemapIterImpl : public lvc::vfs::detail::DirIterImpl {
  std::string Dir;
  lvc::sys::path::Style DirStyle;
  lvc::vfs::directory_iterator ExternalIter;

public:
  RedirectingFSDirRemapIterImpl(std::string DirPath,
                                lvc::vfs::directory_iterator ExtIter)
      : Dir(std::move(DirPath)), DirStyle(getExistingStyle(Dir)),
        ExternalIter(ExtIter) {
    if (ExternalIter != lvc::vfs::directory_iterator())
      setCurrentEntry();
  }

  void setCurrentEntry() {
    StringRef ExternalPath = ExternalIter->path();
    lvc::sys::path::Style ExternalStyle = getExistingStyle(ExternalPath);
    StringRef File = lvc::sys::path::filename(ExternalPath, ExternalStyle);

    SmallString<128> NewPath(Dir);
    lvc::sys::path::append(NewPath, DirStyle, File);
  ...
  std::error_code increment() override {
    ...
    if (!EC && ExternalIter != lvc::vfs::directory_iterator())
      setCurrentEntry();
    else
      CurrentEntry = directory_entry();
    return EC;
...
std::error_code RedirectingFileSystem::makeAbsolute(SmallVectorImpl<char> &Path) const {
  if (lvc::sys::path::is_absolute(Path, lvc::sys::path::Style::posix) ||
      lvc::sys::path::is_absolute(Path, lvc::sys::path::Style::windows))
    return {};
...
void RedirectingFileSystem::dumpEntry(raw_ostream &OS,
  ...
  if (E->getKind() == RedirectingFileSystem::EK_Directory) {
    ...
    for (std::unique_ptr<Entry> &SubEntry :
         lvc::make_range(DE->contents_begin(), DE->contents_end()))
      dumpEntry(OS, SubEntry.get(), NumSpaces + 2);
  }
}

#if !defined(NDEBUG) || defined(LVC_ENABLE_DUMP)
LVC_DUMP_METHOD void RedirectingFileSystem::dump() const { dump(dbgs()); }
#endif

/// A helper class to hold the common YAML parsing state.
class lvc::vfs::RedirectingFileSystemParser {
  yaml::Stream &Stream;
    ...
    for (auto &I : *M) {
      ...
      if (Key == "name") {
        ...
        if (FS->IsRelativeOverlay) {
          ...
          lvc::sys::path::append(FullPath, Value);
        } else {
          FullPath = Value;
        }
        ...
        lvc_unreachable("key missing from Keys");
      }
    }
...
RedirectingFileSystem::create(std::unique_ptr<MemoryBuffer> Buffer,
  ...
  if (!YAMLFilePath.empty()) {
    ...
    std::error_code EC = lvc::sys::fs::make_absolute(OverlayAbsDir);
...
std::unique_ptr<RedirectingFileSystem> RedirectingFileSystem::create(
  ...
  for (auto &Mapping : lvc::reverse(RemappedFiles)) {
    ...
    StringRef FromDirectory = lvc::sys::path::parent_path(From);
    for (auto I = lvc::sys::path::begin(FromDirectory),
              E = lvc::sys::path::end(FromDirectory);
         I != E; ++I) {
...
bool RedirectingFileSystem::shouldFallBackToExternalFS(
  ...
  if (E && !isa<RedirectingFileSystem::DirectoryRemapEntry>(E))
    return false;
  return shouldUseExternalFS() && EC == lvc::errc::no_such_file_or_directory;
}
...
RedirectingFileSystem::makeCanonical(SmallVectorImpl<char> &Path) const {
  ...
  lvc::SmallString<256> CanonicalPath =
...
RedirectingFileSystem::lookupPathImpl(
  ...
  if (isa<RedirectingFileSystem::FileEntry>(From))
    return make_error_code(lvc::errc::not_a_directory);
...
public:
  ...
  ErrorOr<std::unique_ptr<lvc::MemoryBuffer>>
...
static void getVFSEntries(RedirectingFileSystem::Entry *SrcE,
  ...
  if (Kind == RedirectingFileSystem::EK_DirectoryRemap) {
    ...
    for (auto &Comp : Path)
      lvc::sys::path::append(VPath, Comp);
    Entries.push_back(
...
public:
  JSONWriter(lvc::raw_ostream &OS) : OS(OS) {}
...
void JSONWriter::startDirectory(StringRef Path) {
  ...
  OS.indent(Indent + 2) << "'name': \"" << lvc::yaml::escape(Name) << "\",\n";
...
void JSONWriter::writeEntry(StringRef VPath, StringRef RPath) {
  ...
  OS.indent(Indent + 2) << "'name': \"" << lvc::yaml::escape(VPath) << "\",\n";
  OS.indent(Indent + 2) << "'external-contents': \""
                        << lvc::yaml::escape(RPath) << "\"\n";
  OS.indent(Indent) << "}";
}
...
void YAMLVFSWriter::write(lvc::raw_ostream &OS) {
  lvc::sort(Mappings, [](const YAMLVFSEntry &LHS, const YAMLVFSEntry &RHS) {
