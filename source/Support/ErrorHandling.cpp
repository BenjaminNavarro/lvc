...
#include "lvc/Support/Signals.h"
#include "lvc/Support/Threading.h"
#include "lvc/Support/WindowsError.h"
...
void lvc::install_fatal_error_handler(fatal_error_handler_t handler,
                                       void *user_data) {
#if LVC_ENABLE_THREADS == 1
...
void lvc::remove_fatal_error_handler() {
#if LVC_ENABLE_THREADS == 1
  std::lock_guard<std::mutex> Lock(ErrorHandlerMutex);
#endif
...
void lvc::report_fatal_error(StringRef Reason, bool GenCrashDiag) {
  report_fatal_error(Twine(Reason), GenCrashDiag);
}

void lvc::report_fatal_error(const Twine &Reason, bool GenCrashDiag) {
...
void lvc::install_bad_alloc_error_handler(fatal_error_handler_t handler,
                                           void *user_data) {
#if LVC_ENABLE_THREADS == 1
...
void lvc::remove_bad_alloc_error_handler() {
#if LVC_ENABLE_THREADS == 1
  std::lock_guard<std::mutex> Lock(BadAllocErrorHandlerMutex);
#endif
...
void lvc::report_bad_alloc_error(const char *Reason, bool GenCrashDiag) {
  ...
  if (Handler) {
    ...
    lvc_unreachable("bad alloc handler should not return");
  }
...
void lvc::install_out_of_memory_new_handler() {
}
#else
// Causes crash on allocation failure. It is called prior to the handler set by
// 'install_bad_alloc_error_handler'.
static void out_of_memory_new_handler() {
  lvc::report_bad_alloc_error("Allocation failed");
}
...
void lvc::lvc_unreachable_internal(const char *msg, const char *file,
                                     unsigned line) {
  // This code intentionally doesn't call the ErrorHandler callback, because
  // lvc_unreachable is intended to be used to indicate "impossible"
...
static void bindingsErrorHandler(void *user_data, const std::string& reason,
  ...
  LLVMFatalErrorHandler handler =
      LVC_EXTENSION reinterpret_cast<LLVMFatalErrorHandler>(user_data);
  handler(reason.c_str());
}

void LLVMInstallFatalErrorHandler(LLVMFatalErrorHandler Handler) {
  install_fatal_error_handler(bindingsErrorHandler,
                              LVC_EXTENSION reinterpret_cast<void *>(Handler));
}
...
std::error_code lvc::mapWindowsError(unsigned EV) {
  switch (EV) {
