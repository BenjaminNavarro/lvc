...
unsigned StringMapImpl::LookupBucketFor(StringRef Name) {
  ...
  while (true) {
    ...
    } else if (LVC_LIKELY(HashTable[BucketNo] == FullHashValue)) {
...
unsigned StringMapImpl::RehashTable(unsigned BucketNo) {
  ...
  if (LVC_UNLIKELY(NumItems * 4 > NumBuckets * 3)) {
    NewSize = NumBuckets * 2;
  } else if (LVC_UNLIKELY(NumBuckets - (NumItems + NumTombstones) <=
                           NumBuckets / 8)) {
    NewSize = NumBuckets;
  } else {
    return BucketNo;
