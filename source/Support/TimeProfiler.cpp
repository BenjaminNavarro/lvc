...
#include "lvc/Support/TimeProfiler.h"
...
// Per Thread instance
static LVC_THREAD_LOCAL TimeTraceProfiler *TimeTraceProfilerInstance = nullptr;

TimeTraceProfiler *lvc::getTimeTraceProfilerInstance() {
...
struct lvc::TimeTraceProfiler {
  TimeTraceProfiler(unsigned TimeTraceGranularity = 0, StringRef ProcName = "")
      : BeginningOfTime(system_clock::now()), StartTime(steady_clock::now()),
        ProcName(ProcName), Pid(sys::Process::getProcessId()),
        Tid(lvc::get_threadid()), TimeTraceGranularity(TimeTraceGranularity) {
    lvc::get_thread_name(ThreadName);
  }

  void begin(std::string Name, lvc::function_ref<std::string()> Detail) {
  ...
  void write(raw_pwrite_stream &OS) {
    ...
    assert(lvc::all_of(ThreadTimeTraceProfilerInstances,
    ...
    lvc::sort(SortedTotals, [](const NameAndCountAndDurationType &A,
...
void lvc::timeTraceProfilerInitialize(unsigned TimeTraceGranularity,
                                       StringRef ProcName) {
  assert(TimeTraceProfilerInstance == nullptr &&
         "Profiler should not be initialized");
  TimeTraceProfilerInstance = new TimeTraceProfiler(
      TimeTraceGranularity, lvc::sys::path::filename(ProcName));
}

// Removes all TimeTraceProfilerInstances.
// Called from main thread.
void lvc::timeTraceProfilerCleanup() {
...
void lvc::timeTraceProfilerFinishThread() {
  std::lock_guard<std::mutex> Lock(Mu);
  ThreadTimeTraceProfilerInstances.push_back(TimeTraceProfilerInstance);
  TimeTraceProfilerInstance = nullptr;
}

void lvc::timeTraceProfilerWrite(raw_pwrite_stream &OS) {
  assert(TimeTraceProfilerInstance != nullptr &&
         "Profiler object can't be null");
  TimeTraceProfilerInstance->write(OS);
}

Error lvc::timeTraceProfilerWrite(StringRef PreferredFileName,
...
void lvc::timeTraceProfilerBegin(StringRef Name, StringRef Detail) {
  if (TimeTraceProfilerInstance != nullptr)
    TimeTraceProfilerInstance->begin(std::string(Name),
                                     [&]() { return std::string(Detail); });
}

void lvc::timeTraceProfilerBegin(StringRef Name,
                                  lvc::function_ref<std::string()> Detail) {
  if (TimeTraceProfilerInstance != nullptr)
    TimeTraceProfilerInstance->begin(std::string(Name), Detail);
}

void lvc::timeTraceProfilerEnd() {
  if (TimeTraceProfilerInstance != nullptr)
