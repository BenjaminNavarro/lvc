...
hash_code lvc::hash_value(const APInt &Arg) {
  if (Arg.isSingleWord())
...
unsigned APInt::countLeadingOnesSlowCase() const {
  ...
  int i = getNumWords() - 1;
  unsigned Count = lvc::countLeadingOnes(U.pVal[i] << shift);
  if (Count == highWordBits) {
    for (i--; i >= 0; --i) {
      if (U.pVal[i] == WORDTYPE_MAX)
        Count += APINT_BITS_PER_WORD;
      else {
        Count += lvc::countLeadingOnes(U.pVal[i]);
        break;
      }
    }
  }
...
unsigned APInt::countTrailingZerosSlowCase() const {
  ...
  for (; i < getNumWords() && U.pVal[i] == 0; ++i)
    Count += APINT_BITS_PER_WORD;
  if (i < getNumWords())
    Count += lvc::countTrailingZeros(U.pVal[i]);
...
unsigned APInt::countTrailingOnesSlowCase() const {
  ...
  for (; i < getNumWords() && U.pVal[i] == WORDTYPE_MAX; ++i)
    Count += APINT_BITS_PER_WORD;
  if (i < getNumWords())
    Count += lvc::countTrailingOnes(U.pVal[i]);
...
unsigned APInt::countPopulationSlowCase() const {
  ...
  for (unsigned i = 0; i < getNumWords(); ++i)
    Count += lvc::countPopulation(U.pVal[i]);
  return Count;
}
...
APInt APInt::reverseBits() const {
  switch (BitWidth) {
  case 64:
    return APInt(BitWidth, lvc::reverseBits<uint64_t>(U.VAL));
  case 32:
    return APInt(BitWidth, lvc::reverseBits<uint32_t>(U.VAL));
  case 16:
    return APInt(BitWidth, lvc::reverseBits<uint16_t>(U.VAL));
  case 8:
    return APInt(BitWidth, lvc::reverseBits<uint8_t>(U.VAL));
  default:
    break;
  }
...
APInt lvc::APIntOps::GreatestCommonDivisor(APInt A, APInt B) {
...
APInt lvc::APIntOps::RoundDoubleToAPInt(double Double, unsigned width) {
...
#ifdef KNUTH_DEBUG
#define DEBUG_KNUTH(X) LVC_DEBUG(X)
#else
...
LVC_DUMP_METHOD void APInt::dump() const {
  SmallString<40> S, U;
...
APInt lvc::APIntOps::RoundingUDiv(const APInt &A, const APInt &B,
                                   APInt::Rounding RM) {
  ...
  lvc_unreachable("Unknown APInt::Rounding enum");
}

APInt lvc::APIntOps::RoundingSDiv(const APInt &A, const APInt &B,
                                   APInt::Rounding RM) {
  switch (RM) {
  case APInt::Rounding::DOWN:
  case APInt::Rounding::UP: {
...
Optional<APInt>
lvc::APIntOps::SolveQuadraticEquationWrap(APInt A, APInt B, APInt C,
                                           unsigned RangeWidth) {
  ...
  LVC_DEBUG(dbgs() << __func__ << ": updated coefficients " << A << "x^2 + "
                    << B << "x + " << C << ", rw:" << RangeWidth << '\n');
  ...
  if (!InexactSQ && Rem.isNullValue()) {
    LVC_DEBUG(dbgs() << __func__ << ": solution (root): " << X << '\n');
    return X;
  }
  ...
  if (!SignChange) {
    LVC_DEBUG(dbgs() << __func__ << ": no valid solution\n");
    return None;
  }

  X += 1;
  LVC_DEBUG(dbgs() << __func__ << ": solution (wrap): " << X << '\n');
  return X;
}

Optional<unsigned>
lvc::APIntOps::GetMostSignificantDifferentBit(const APInt &A, const APInt &B) {
...
void lvc::StoreIntToMemory(const APInt &IntVal, uint8_t *Dst,
                            unsigned StoreBytes) {
...
void lvc::LoadIntFromMemory(APInt &IntVal, const uint8_t *Src,
                             unsigned LoadBytes) {
