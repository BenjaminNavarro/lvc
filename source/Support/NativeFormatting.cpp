...
void lvc::write_integer(raw_ostream &S, unsigned int N, size_t MinDigits,
                         IntegerStyle Style) {
  write_unsigned(S, N, MinDigits, Style);
}

void lvc::write_integer(raw_ostream &S, int N, size_t MinDigits,
                         IntegerStyle Style) {
  write_signed(S, N, MinDigits, Style);
}

void lvc::write_integer(raw_ostream &S, unsigned long N, size_t MinDigits,
                         IntegerStyle Style) {
  write_unsigned(S, N, MinDigits, Style);
}

void lvc::write_integer(raw_ostream &S, long N, size_t MinDigits,
                         IntegerStyle Style) {
  write_signed(S, N, MinDigits, Style);
}

void lvc::write_integer(raw_ostream &S, unsigned long long N, size_t MinDigits,
                         IntegerStyle Style) {
  write_unsigned(S, N, MinDigits, Style);
}

void lvc::write_integer(raw_ostream &S, long long N, size_t MinDigits,
                         IntegerStyle Style) {
  write_signed(S, N, MinDigits, Style);
}

void lvc::write_hex(raw_ostream &S, uint64_t N, HexPrintStyle Style,
  ...
  ::memset(NumberBuffer, '0', lvc::array_lengthof(NumberBuffer));
  if (Prefix)
    NumberBuffer[1] = 'x';
...
void lvc::write_double(raw_ostream &S, double N, FloatStyle Style,
  ...
  SmallString<8> Spec;
  lvc::raw_svector_ostream Out(Spec);
  Out << "%." << Prec << Letter;
...
bool lvc::isPrefixedHexStyle(HexPrintStyle S) {
  return (S == HexPrintStyle::PrefixLower || S == HexPrintStyle::PrefixUpper);
}

size_t lvc::getDefaultPrecision(FloatStyle Style) {
  switch (Style) {
