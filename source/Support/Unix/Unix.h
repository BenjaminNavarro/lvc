...
#ifndef LVC_LIB_SUPPORT_UNIX_UNIX_H
#define LVC_LIB_SUPPORT_UNIX_UNIX_H
...
static inline bool MakeErrMsg(
  ...
  if (errnum == -1)
    errnum = errno;
  *ErrMsg = prefix + ": " + lvc::sys::StrError(errnum);
  return true;
}

// Include StrError(errnum) in a fatal error message.
LVC_ATTRIBUTE_NORETURN static inline void ReportErrnumFatal(const char *Msg,
                                                             int errnum) {
  std::string ErrMsg;
