...
#include "lvc/Support/DynamicLibrary.h"
...
public:
  ...
  void *LibLookup(const char *Symbol, DynamicLibrary::SearchOrdering Order) {
    if (Order & SO_LoadOrder) {
      ...
      for (void *Handle : lvc::reverse(Handles)) {
        if (void *Ptr = DLSym(Handle, Symbol))
          return Ptr;
      }
    }
...
static lvc::ManagedStatic<lvc::StringMap<void *>> ExplicitSymbols;
// Collection of known library handles.
static lvc::ManagedStatic<DynamicLibrary::HandleSet> OpenedHandles;
// Lock for ExplicitSymbols and OpenedHandles.
static lvc::ManagedStatic<lvc::sys::SmartMutex<true>> SymbolsMutex;
} // namespace
...
void *DynamicLibrary::SearchForAddressOfSymbol(const char *SymbolName) {
  ...
  return lvc::SearchForAddressOfSpecialSymbol(SymbolName);
}
...
LLVMBool LLVMLoadLibraryPermanently(const char *Filename) {
  return lvc::sys::DynamicLibrary::LoadLibraryPermanently(Filename);
}

void *LLVMSearchForAddressOfSymbol(const char *symbolName) {
  return lvc::sys::DynamicLibrary::SearchForAddressOfSymbol(symbolName);
}

void LLVMAddSymbol(const char *symbolName, void *symbolValue) {
  return lvc::sys::DynamicLibrary::AddSymbol(symbolName, symbolValue);
}
