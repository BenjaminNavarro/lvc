...
static LVC_THREAD_LOCAL PrettyStackTraceEntry *PrettyStackTraceHead = nullptr;
...
static volatile std::atomic<unsigned> GlobalSigInfoGenerationCounter =
    ATOMIC_VAR_INIT(1);
static LVC_THREAD_LOCAL unsigned ThreadLocalSigInfoGenerationCounter = 0;
...
static void setCrashLogMessage(const char *msg) LVC_ATTRIBUTE_UNUSED;
...
void lvc::setBugReportMsg(const char *Msg) {
  BugReportMsg = Msg;
}

const char *lvc::getBugReportMsg() {
  return BugReportMsg;
}
...
void lvc::EnablePrettyStackTrace() {
#if ENABLE_BACKTRACES
...
void lvc::EnablePrettyStackTraceOnSigInfoForThisThread(bool ShouldEnable) {
#if ENABLE_BACKTRACES
  if (!ShouldEnable) {
...
const void *lvc::SavePrettyStackState() {
#if ENABLE_BACKTRACES
...
void lvc::RestorePrettyStackState(const void *Top) {
#if ENABLE_BACKTRACES
