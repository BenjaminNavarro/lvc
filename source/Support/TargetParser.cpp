...
#include "lvc/Support/ARMBuildAttributes.h"
...
const GPUInfo *getArchEntry(AMDGPU::GPUKind AK, ArrayRef<GPUInfo> Table) {
  ...
  auto I =
      lvc::lower_bound(Table, Search, [](const GPUInfo &A, const GPUInfo &B) {
...
StringRef lvc::AMDGPU::getArchNameAMDGCN(GPUKind AK) {
  if (const auto *Entry = getArchEntry(AK, AMDGCNGPUs))
    return Entry->CanonicalName;
  return "";
}

StringRef lvc::AMDGPU::getArchNameR600(GPUKind AK) {
  if (const auto *Entry = getArchEntry(AK, R600GPUs))
    return Entry->CanonicalName;
  return "";
}

AMDGPU::GPUKind lvc::AMDGPU::parseArchAMDGCN(StringRef CPU) {
...
AMDGPU::GPUKind lvc::AMDGPU::parseArchR600(StringRef CPU) {
...
#include "lvc/Support/RISCVTargetParser.def"
};
...
CPUKind parseCPUKind(StringRef CPU) {
  return lvc::StringSwitch<CPUKind>(CPU)
...
StringRef resolveTuneCPUAlias(StringRef TuneCPU, bool IsRV64) {
  return lvc::StringSwitch<StringRef>(TuneCPU)
...
CPUKind parseTuneCPUKind(StringRef TuneCPU, bool IsRV64) {
  ...
  return lvc::StringSwitch<CPUKind>(TuneCPU)
#define PROC(ENUM, NAME, FEATURES, DEFAULT_MARCH) .Case(NAME, CK_##ENUM)
#include "lvc/Support/RISCVTargetParser.def"
      .Default(CK_INVALID);
}
