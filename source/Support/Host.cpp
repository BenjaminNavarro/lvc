...
static std::unique_ptr<lvc::MemoryBuffer>
    LVC_ATTRIBUTE_UNUSED getProcCpuinfoContent() {
  lvc::ErrorOr<std::unique_ptr<lvc::MemoryBuffer>> Text =
      lvc::MemoryBuffer::getFileAsStream("/proc/cpuinfo");
...
using namespace lvc::sys::detail::x86;
...
StringRef sys::getHostCPUName() {
  std::unique_ptr<lvc::MemoryBuffer> P = getProcCpuinfoContent();
...
#elif defined(_WIN32) && LVC_ENABLE_THREADS != 0
...
std::string sys::getProcessTriple() {
  std::string TargetTripleString = updateTripleOSVersion(LVC_HOST_TRIPLE);
