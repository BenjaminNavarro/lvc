...
#include "lvc/ADT/DAGDeltaAlgorithm.h"
#include "lvc/ADT/DeltaAlgorithm.h"
...
DAGDeltaAlgorithmImpl::DAGDeltaAlgorithmImpl(
  ...
  LVC_DEBUG({
    lvc::errs() << "-- DAGDeltaAlgorithmImpl --\n";
    lvc::errs() << "Changes: [";
    ...
    lvc::errs() << "Predecessor Closure:\n";
    for (changeset_ty::const_iterator it = Changes.begin(), ie = Changes.end();
         it != ie; ++it) {
      lvc::errs() << format("  %-4d: [", *it);
...
DAGDeltaAlgorithmImpl::Run() {
  ...
  while (!CurrentSet.empty()) {
    LVC_DEBUG({
      lvc::errs() << "DAG_DD - " << CurrentSet.size() << " active changes, "
