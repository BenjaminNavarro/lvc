...
#if LVC_SUPPORT_XCODE_SIGNPOSTS
#include "lvc/ADT/DenseMap.h"
#include "lvc/Support/Mutex.h"
#include <Availability.h>
#include <os/signpost.h>
#endif // if LVC_SUPPORT_XCODE_SIGNPOSTS
...
public:
  ...
  void startInterval(const void *O, lvc::StringRef Name) {
    if (isEnabled()) {
      if (SIGNPOSTS_AVAILABLE()) {
  ...
  void endInterval(const void *O, lvc::StringRef Name) {
    if (isEnabled()) {
      if (SIGNPOSTS_AVAILABLE()) {
        // Both strings used here are required to be constant literal strings.
