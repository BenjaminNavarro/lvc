...
#include "lvc/Support/FileCollector.h"
...
lvc::vfs::directory_iterator
FileCollector::addDirectoryImpl(const lvc::Twine &Dir,
  ...
  if (EC)
    return It;
  addFile(Dir);
  for (; !EC && It != lvc::vfs::directory_iterator(); It.increment(EC)) {
...
public:
  ...
  lvc::ErrorOr<lvc::vfs::Status> status(const Twine &Path) override {
  ...
  lvc::ErrorOr<std::unique_ptr<lvc::vfs::File>>
  ...
  lvc::vfs::directory_iterator dir_begin(const lvc::Twine &Dir,
  ...
  std::error_code setCurrentWorkingDirectory(const lvc::Twine &Path) override {
