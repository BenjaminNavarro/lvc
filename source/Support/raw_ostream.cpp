...
#include "lvc/Support/NativeFormatting.h"
#include "lvc/Support/Process.h"
#include "lvc/Support/Program.h"
#include <algorithm>
#include <cctype>
#include <cerrno>
#include <cstdio>
...
#include "lvc/Support/Windows/WindowsSupport.h"
#endif
...
raw_ostream &raw_ostream::write_hex(unsigned long long N) {
  lvc::write_hex(*this, N, HexPrintStyle::Lower);
  return *this;
}
...
raw_ostream &raw_ostream::operator<<(const void *P) {
  lvc::write_hex(*this, (uintptr_t)P, HexPrintStyle::PrefixLower);
  return *this;
}

raw_ostream &raw_ostream::operator<<(double N) {
  lvc::write_double(*this, N, FloatStyle::Exponent);
  return *this;
}
...
raw_ostream &raw_ostream::write(unsigned char C) {
  ...
  if (LVC_UNLIKELY(OutBufCur >= OutBufEnd)) {
    if (LVC_UNLIKELY(!OutBufStart)) {
...
raw_ostream &raw_ostream::write(const char *Ptr, size_t Size) {
  // Group exceptional cases into a single branch.
  if (LVC_UNLIKELY(size_t(OutBufEnd - OutBufCur) < Size)) {
    if (LVC_UNLIKELY(!OutBufStart)) {
      if (BufferMode == BufferKind::Unbuffered) {
    ...
    if (LVC_UNLIKELY(OutBufCur == OutBufStart)) {
...
void raw_ostream::copy_to_buffer(const char *Ptr, size_t Size) {
  ...
  // strings.
  switch (Size) {
  case 4: OutBufCur[3] = Ptr[3]; LVC_FALLTHROUGH;
  case 3: OutBufCur[2] = Ptr[2]; LVC_FALLTHROUGH;
  case 2: OutBufCur[1] = Ptr[1]; LVC_FALLTHROUGH;
  case 1: OutBufCur[0] = Ptr[0]; LVC_FALLTHROUGH;
  case 0: break;
  default:
...
raw_ostream &raw_ostream::operator<<(const FormattedNumber &FN) {
  if (FN.Hex) {
    ...
    lvc::write_hex(*this, FN.HexValue, Style, FN.Width);
  } else {
    lvc::SmallString<16> Buffer;
    lvc::raw_svector_ostream Stream(Buffer);
    lvc::write_integer(Stream, FN.DecValue, 0, IntegerStyle::Integer);
...
raw_ostream &raw_ostream::operator<<(const FormattedBytes &FB) {
  ...
  if (FB.FirstByteOffset.hasValue()) {
    ...
    unsigned Power = 0;
    if (MaxOffset > 0)
      Power = lvc::Log2_64_Ceil(MaxOffset);
    OffsetWidth = std::max<uint64_t>(4, lvc::alignTo(Power, 4) / 4);
  }
  ...
  while (!Bytes.empty()) {
    ...
    if (FB.FirstByteOffset.hasValue()) {
      ...
      lvc::write_hex(*this, Offset + LineIndex, HPS, OffsetWidth);
      *this << ": ";
    }
    ...
    for (size_t I = 0; I < Line.size(); ++I, CharsPrinted += 2) {
      ...
      lvc::write_hex(*this, Line[I], HPS, 2);
    }
...
Error lvc::writeToOutput(StringRef OutputFileName,
